...
export class LocalTranscriptionService {
  ...
  async start(input: { sessionId: string; noteId: string; workspaceId: string; force?: boolean }) {
    ...
    if (input.force) this.jobs.update(job.jobId, { status: 'queued', progress: 0, error: null, completedChunks: 0, currentSource: null, currentChunkSequence: null, segments: [], skippedChunks: [], startedAt: null, completedAt: null }, 'job_restarted');
  ...
  cancel(jobId: string) {
    ...
    this.jobs.update(jobId, { status: 'cancelled', error: 'Transcription cancelled.' }, 'job_cancelled');
  ...
  complete(jobId: string, retention: 'delete_after_transcription' | 'retain') {
    ...
    this.jobs.update(jobId, { status: 'complete', progress: 1, completedAt: new Date().toISOString(), error: null }, 'job_complete');
    return this.publicJob(jobId);
  }

  fail(jobId: string, error: string) { return this.jobs.update(jobId, { status: 'failed', error: error.slice(0, 500), completedAt: new Date().toISOString() }, 'job_failed'); }
  ...
  private async run(jobId: string) {
    ...
    this.jobs.update(jobId, { status: 'preparing', startedAt: new Date().toISOString(), totalChunks: chunks.length, error: null }, 'job_started');
    try {
      ...
      for (let index = job.completedChunks; index < chunks.length; index += 1) {
        ...
        this.jobs.update(jobId, { status: 'transcribing', currentSource: chunk.source, currentChunkSequence: chunk.sequence, progress: index / chunks.length }, 'chunk_started');
        ...
        this.jobs.update(jobId, { segments: dedupeSegments(merged), completedChunks: index + 1, progress: (index + 1) / chunks.length }, 'chunk_complete');
      }
      this.jobs.update(jobId, { status: 'merging', progress: 1, currentSource: null, currentChunkSequence: null }, 'job_merging');
      this.emit(jobId);
    } catch (error) {
  ...
  private transcribeChunk(job: TranscriptionJob, chunk: RecordingChunk, sessionStartedAt: number) {
    const directory = path.join(app.getPath('temp'), 'ledger-meeting-audio', job.sessionId);
    ...
    if (isSilentWav(input)) {
      this.jobs.update(job.jobId, { skippedChunks: [...job.skippedChunks, chunk.sequence] }, 'silent_chunk_skipped');
    ...
    return new Promise<LocalTranscriptSegment[]>((resolve, reject) => {
      ...
      child.once('exit', (code) => {
        ...
        try {
          ...
          if (!rows.length) this.jobs.update(job.jobId, { skippedChunks: [...job.skippedChunks, chunk.sequence] }, 'silent_chunk_skipped');
          resolve(rows);
  ...
  private runtimePath() {
    const candidates = [process.env.LEDGER_WHISPER_CLI, app.isPackaged ? path.join(process.resourcesPath, 'whisper-cli') : path.join(app.getAppPath(), 'native', 'whisper-cli')].filter(Boolean) as string[];
    const found = candidates.find((candidate) => fs.existsSync(candidate));
    if (!found) throw new Error('The local Whisper runtime is not installed in this Ledger build.');
    return found;
  }
  private deleteSessionAudio(sessionId: string) { fs.rmSync(path.join(app.getPath('temp'), 'ledger-meeting-audio', sessionId), { recursive: true, force: true }); }
