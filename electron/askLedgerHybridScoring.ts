...
export const lexicalMatch = (question: string, item: AskLedgerContextItem) => {
  ...
  const searchable = normalizeHybridText([
    item.title,
    item.content,
    item.projectName,
    item.containerName,
  ...
  return {
    score: Math.min(1, titleTokenCoverage * 0.72 + contentCoverage * 0.28 + (phraseMatch ? 0.18 : 0)),
    phraseMatch,
    titleTokenCoverage,
  };
};

export const entityMatch = (entityQuery: string | undefined, item: AskLedgerContextItem) => {
