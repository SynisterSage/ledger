...
export class CloudAIProvider {
  ...
  async stream(provider: AIProvider, request: LocalAIRequest, callbacks: StreamCallbacks, signal: AbortSignal, requestId: string) {
    ...
    try {
      ...
      const endpoint = provider === 'openai' ? 'https://api.openai.com/v1/chat/completions' : provider === 'anthropic' ? 'https://api.anthropic.com/v1/messages' : provider === 'google' ? `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent` : 'https://api.perplexity.ai/chat/completions';
      ...
      const consume = (line: string) => {
        ...
        try {
          ...
          const text = provider === 'openai' || provider === 'perplexity'
