...
const windowsDeviceChangeListener = () =>
...
window.addEventListener('beforeunload', () =>
  navigator.mediaDevices?.removeEventListener('devicechange', windowsDeviceChangeListener)
);

function windowsCaptureError(code: string, message: string, source?: WindowsCaptureSource) {
  return {
    event: 'error',
    sessionId: windowsCaptureRuntimes.keys().next().value ?? '',
    source,
    code,
    error: message,
  };
...
async function flushWindowsSource(
  runtime: WindowsCaptureRuntime,
  source: WindowsCaptureSourceRuntime
) {
  ...
  const data = wav;
  source.flushing = source.flushing
    .then(async () => {
      await ipcRenderer.invoke('meeting-audio:windows-chunk', {
        sessionId: runtime.sessionId,
        source: source.source,
        sequence: source.sequence++,
        startAt,
        endAt,
        durationSeconds: frameCount / source.context.sampleRate,
        data,
      });
    })
    .catch((error) => {
      ipcRenderer.send(
        'meeting-audio:windows-event',
        windowsCaptureError(
          'capture_interrupted',
          error instanceof Error ? error.message : 'Windows audio chunk could not be saved.',
          source.source
        )
      );
    });
...
function encodeFloat32InterleavedBase64(buffers: Float32Array[][], frameCount: number) {
  ...
  const flattened = buffers.map((blocks) => {
    ...
    for (const block of blocks) {
      const length = Math.min(block.length, frameCount - offset);
      if (length > 0) output.set(block.subarray(0, length), offset);
      offset += block.length;
  ...
  for (let frame = 0; frame < frameCount; frame += 1)
    for (let channel = 0; channel < channels; channel += 1)
      view.setFloat32((frame * channels + channel) * 4, flattened[channel]?.[frame] ?? 0, true);
  let binary = '';
...
function flushWindowsLiveSource(
  runtime: WindowsCaptureRuntime,
  source: WindowsCaptureSourceRuntime
) {
  ...
  ipcRenderer.send('meeting-audio:windows-event', {
    event: 'audio-data',
    sessionId: runtime.sessionId,
    source: source.source,
    ...
    data: encodeFloat32InterleavedBase64(buffers, frameCount),
...
async function stopWindowsRuntime(runtime: WindowsCaptureRuntime) {
  ...
  const pending = [...runtime.sources.values()].map((source) =>
    flushWindowsSource(runtime, source)
  );
  await Promise.allSettled(pending);
  runtime.sources.forEach((source) => {
    source.trackListeners.forEach(({ track, listener }) =>
  ...
  [runtime.microphoneStream, runtime.displayStream].forEach((stream) =>
    stream?.getTracks().forEach((track) => track.stop())
  );
  if (runtime.deviceChange)
    navigator.mediaDevices.removeEventListener('devicechange', runtime.deviceChange);
  runtime.streamListeners.forEach(({ stream, listener }) =>
    stream.removeEventListener('inactive', listener)
  );
  windowsCaptureRuntimes.delete(runtime.sessionId);
}

async function startWindowsRuntime(command: {
  sessionId: string;
  microphone: boolean;
  systemAudio: boolean;
  ...
  if (windowsCaptureRuntimes.has(command.sessionId)) {
    ipcRenderer.send('meeting-audio:windows-event', {
      ...
      error: 'Another Windows audio capture session is already active.',
    });
    return;
  }
  const runtime: WindowsCaptureRuntime = {
    sessionId: command.sessionId,
    startedAt: Date.now(),
    paused: false,
    pauseStartedAt: null,
  ...
  try {
    if (command.microphone) {
      try {
        runtime.microphoneStream = await navigator.mediaDevices.getUserMedia({
          audio: command.microphoneDeviceId
        ...
        throw {
          code: /denied|permission/i.test(String(error))
            ? 'microphone_permission_denied'
            : 'no_microphone_available',
          message: 'Ledger could not access the selected Windows microphone.',
        };
      }
      if (!runtime.microphoneStream.getAudioTracks().some((track) => track.readyState === 'live'))
        throw {
          code: 'no_microphone_available',
          message: 'No live Windows microphone track is available.',
        };
    }
    if (command.systemAudio) {
      try {
        runtime.displayStream = await navigator.mediaDevices.getDisplayMedia({
          video: true,
          audio: true,
        });
      } catch {
        throw {
          code: 'display_capture_denied',
          message: 'Windows display capture permission was denied.',
        };
      }
      if (!runtime.displayStream.getAudioTracks().some((track) => track.readyState === 'live'))
        throw {
          code: 'no_output_device_available',
          message: 'Windows returned no live system-audio track from the active output device.',
        };
    }
    const streams: Array<[WindowsCaptureSource, MediaStream | null]> = [
      ['user_microphone', runtime.microphoneStream],
      [
        'system_audio',
        runtime.displayStream ? new MediaStream(runtime.displayStream.getAudioTracks()) : null,
      ],
    ];
    for (const [sourceName, stream] of streams) {
      ...
      gain.gain.value = 0;
      const source: WindowsCaptureSourceRuntime = {
        source: sourceName,
        stream,
        context,
        processor,
        gain,
        buffers: Array.from({ length: processor.channelCount }, () => []),
        frameCount: 0,
        sequence: 0,
        chunkStartedAt: Date.now(),
        flushing: Promise.resolve(),
        liveBuffers: Array.from({ length: processor.channelCount }, () => []),
        liveFrameCount: 0,
      ...
      processor.onaudioprocess = (event) => {
        ...
        for (let channel = 0; channel < source.buffers.length; channel += 1)
          source.buffers[channel].push(
            new Float32Array(input.getChannelData(Math.min(channel, input.numberOfChannels - 1)))
          );
        source.liveFrameCount += input.length;
        for (let channel = 0; channel < source.liveBuffers.length; channel += 1)
          source.liveBuffers[channel].push(
            new Float32Array(input.getChannelData(Math.min(channel, input.numberOfChannels - 1)))
          );
        let energy = 0;
        ...
        ipcRenderer.send('meeting-audio:windows-event', {
          ...
          sessionId: runtime.sessionId,
          source: sourceName,
          level: Math.min(1, Math.sqrt(energy / Math.max(1, samples.length)) * 4),
        });
        if (source.frameCount >= context.sampleRate * 30) void flushWindowsSource(runtime, source);
        if (source.liveFrameCount >= context.sampleRate * 5)
          flushWindowsLiveSource(runtime, source);
      };
      const onEnded = () =>
        ipcRenderer.send(
          'meeting-audio:windows-event',
          windowsCaptureError(
            'device_disconnected',
            `${sourceName === 'system_audio' ? 'System audio' : 'Microphone'} capture ended.`,
            sourceName
          )
        );
      stream.getTracks().forEach((track) => {
        track.addEventListener('ended', onEnded);
        source.trackListeners.push({ track, listener: onEnded });
      });
      if (sourceName === 'system_audio')
        runtime.displayStream?.getVideoTracks().forEach((track) => {
          track.addEventListener('ended', onEnded);
          source.trackListeners.push({ track, listener: onEnded });
        });
      stream.addEventListener('inactive', onEnded);
      runtime.streamListeners.push({ stream, listener: onEnded });
      if (sourceName === 'system_audio' && runtime.displayStream) {
        runtime.displayStream.addEventListener('inactive', onEnded);
        runtime.streamListeners.push({ stream: runtime.displayStream, listener: onEnded });
      }
    ...
    runtime.deviceChange = () => {
      ipcRenderer.send('meeting-audio:windows-event', { event: 'devices-changed' });
      runtime.sources.forEach((source) => {
        if (!source.stream.getAudioTracks().some((track) => track.readyState === 'live'))
          ipcRenderer.send(
            'meeting-audio:windows-event',
            windowsCaptureError(
              'device_disconnected',
              `${
                source.source === 'system_audio' ? 'System audio' : 'Microphone'
              } device disconnected.`,
              source.source
            )
          );
    ...
    const sources = [...runtime.sources.values()].map((source) => ({
      source: source.source,
    ...
    if (!sources.length)
      throw {
        code: 'empty_audio_stream',
        message: 'Windows did not provide a usable audio stream.',
      };
    ipcRenderer.send('meeting-audio:windows-event', {
      event: 'started',
      requestId: command.requestId,
    ...
    ipcRenderer.send('meeting-audio:windows-event', {
      ...
      requestId: command.requestId,
      sessionId: command.sessionId,
      code: detail.code || 'capture_initialization_failed',
      error: detail.message || 'Windows audio capture could not start.',
    });
  }
}

async function handleWindowsCaptureCommand(command: {
  command: string;
  requestId: string;
  sessionId: string;
  microphone?: boolean;
  ...
  if (command.command === 'start')
    return startWindowsRuntime({
      ...command,
      microphone: command.microphone !== false,
  ...
  if (command.command === 'devices') {
    ...
    const inputs = devices
      ...
      .map((device) => ({
        ...
        kind: 'input' as const,
        available: true,
        isBluetooth: /bluetooth|headset|airpods/i.test(device.label),
        isDefault: device.deviceId === 'default',
        isOutputDefault: false,
        channelCount: 1,
      }));
    ...
    ipcRenderer.send('meeting-audio:windows-event', {
      ...
      outputDevice: output
        ? {
            id: output.deviceId,
            name: output.label || 'Current output',
            isBluetooth: /bluetooth|headset|airpods/i.test(output.label),
          }
        : null,
      sources: [],
      warnings: [],
  ...
  if (!runtime) {
    ipcRenderer.send('meeting-audio:windows-event', {
      ...
      sessionId: command.sessionId,
      code: 'invalid_state',
      error: 'Windows audio capture is not active.',
    });
    return;
  }
  if (command.command === 'pause') {
    await Promise.all(
      [...runtime.sources.values()].map((source) => flushWindowsSource(runtime, source))
    );
    runtime.paused = true;
    runtime.pauseStartedAt = Date.now();
    ipcRenderer.send('meeting-audio:windows-event', {
      event: 'paused',
      requestId: command.requestId,
      sessionId: command.sessionId,
  ...
  if (command.command === 'resume') {
    if (runtime.pauseStartedAt) runtime.pausedMilliseconds += Date.now() - runtime.pauseStartedAt;
  ...
  if (command.command === 'health') {
    const sources = [...runtime.sources.values()].map((source) => ({
      ...
      sampleRate: TARGET_SAMPLE_RATE,
      channels: 1,
      active: source.stream.getAudioTracks().some((track) => track.readyState === 'live'),
    }));
    ipcRenderer.send('meeting-audio:windows-event', {
      event: 'health',
      requestId: command.requestId,
  ...
  if (command.command === 'stop') {
    ...
    ipcRenderer.send('meeting-audio:windows-event', {
      ...
      durationSeconds: Math.max(
        0,
        (Date.now() - runtime.startedAt - runtime.pausedMilliseconds) / 1000
      ),
    });
  }
}
...
ipcRenderer.on(
  'meeting-audio:windows-command',
  (_event, command: Parameters<typeof handleWindowsCaptureCommand>[0]) => {
    void handleWindowsCaptureCommand(command);
  }
);
...
const unsubscribeFromLedgerEvent = (channel: string, subscriptionId: unknown) => {
  ...
  const token =
    typeof subscriptionId === 'string' ? subscriptionId : [...channelListeners.keys()].at(-1);
...
const ledgerEventChannels = {
  onCalendarFollowUpCreated: 'calendar:follow-up-created',
  offCalendarFollowUpCreated: 'calendar:follow-up-created',
  onCalendarItemsUpdated: 'calendar:items-updated',
  offCalendarItemsUpdated: 'calendar:items-updated',
  onDailyCheckinUpdated: 'daily:checkin-updated',
  offDailyCheckinUpdated: 'daily:checkin-updated',
  onDashboardTodayTaskCreated: 'dashboard:today-task-created',
  offDashboardTodayTaskCreated: 'dashboard:today-task-created',
  onDashboardTodayTaskDeleted: 'dashboard:today-task-deleted',
  offDashboardTodayTaskDeleted: 'dashboard:today-task-deleted',
  onInboxItemsUpdated: 'inbox:items-updated',
  offInboxItemsUpdated: 'inbox:items-updated',
  onLedgerNotificationsBatch: 'ledger:notifications-batch',
  offLedgerNotificationsBatch: 'ledger:notifications-batch',
  onLedgerNotificationsSummary: 'ledger:notifications-summary',
  offLedgerNotificationsSummary: 'ledger:notifications-summary',
  onLedgerNotificationsSessionInvalid: 'ledger:notifications-session-invalid',
  offLedgerNotificationsSessionInvalid: 'ledger:notifications-session-invalid',
  onLedgerOpenInvite: 'ledger:open-invite',
  offLedgerOpenInvite: 'ledger:open-invite',
  onLedgerSetActiveWorkspace: 'ledger:set-active-workspace',
  offLedgerSetActiveWorkspace: 'ledger:set-active-workspace',
  onLedgerThemeUpdated: 'ledger:theme-updated',
  offLedgerThemeUpdated: 'ledger:theme-updated',
  onModuleFocusContext: 'module:focus-context',
  offModuleFocusContext: 'module:focus-context',
  onModuleFocusInbox: 'module:focus-inbox',
  offModuleFocusInbox: 'module:focus-inbox',
  onModuleFocusNote: 'module:focus-note',
  offModuleFocusNote: 'module:focus-note',
  onModuleFocusProject: 'module:focus-project',
  offModuleFocusProject: 'module:focus-project',
  onModuleFocusSection: 'module:focus-section',
  offModuleFocusSection: 'module:focus-section',
  onModuleFocusTask: 'module:focus-task',
  offModuleFocusTask: 'module:focus-task',
  onModuleFullscreenStateChanged: 'module:fullscreen-state-changed',
  offModuleFullscreenStateChanged: 'module:fullscreen-state-changed',
  onModuleStateChanged: 'module:state-changed',
  offModuleStateChanged: 'module:state-changed',
  onNotesSmartLinksUpdated: 'notes:smart-links-updated',
  offNotesSmartLinksUpdated: 'notes:smart-links-updated',
  onSearchOpen: 'search:open',
  offSearchOpen: 'search:open',
  onSettingsFocusSection: 'settings:focus-section',
  offSettingsFocusSection: 'settings:focus-section',
  onSettingsGithubCallback: 'settings:github-callback',
  offSettingsGithubCallback: 'settings:github-callback',
  onSidebarAccessibilityUpdated: 'sidebar:accessibility-updated',
  offSidebarAccessibilityUpdated: 'sidebar:accessibility-updated',
  onSidebarFloatingDockChanged: 'sidebar:floating-dock-changed',
  offSidebarFloatingDockChanged: 'sidebar:floating-dock-changed',
  onSidebarMaterialState: 'sidebar:material-state',
  offSidebarMaterialState: 'sidebar:material-state',
  onSidebarOpacityPreview: 'sidebar:opacity-preview',
  offSidebarOpacityPreview: 'sidebar:opacity-preview',
  onSidebarOpenCheckin: 'sidebar:open-checkin',
  offSidebarOpenCheckin: 'sidebar:open-checkin',
  onSidebarPreferencesUpdated: 'sidebar:preferences-updated',
  offSidebarPreferencesUpdated: 'sidebar:preferences-updated',
  onSidebarStateChanged: 'sidebar:state-changed',
  offSidebarStateChanged: 'sidebar:state-changed',
  onSidebarVisibilityChanged: 'sidebar:visibility-changed',
  offSidebarVisibilityChanged: 'sidebar:visibility-changed',
  onSlackConnectionChanged: 'slack:connection-changed',
  offSlackConnectionChanged: 'slack:connection-changed',
  onSlackIdentityChanged: 'slack:identity-changed',
  offSlackIdentityChanged: 'slack:identity-changed',
  onSpellcheckContextMenu: 'spellcheck:context-menu',
  offSpellcheckContextMenu: 'spellcheck:context-menu',
  onTabHydrateSession: 'tab:hydrate-session',
  offTabHydrateSession: 'tab:hydrate-session',
  onTouchbarOpenSearch: 'touchbar:open-search',
  offTouchbarOpenSearch: 'touchbar:open-search',
  onTouchbarAction: 'touchbar:action',
  offTouchbarAction: 'touchbar:action',
  onWorkspaceNavigationState: 'workspace:navigation-state',
  offWorkspaceNavigationState: 'workspace:navigation-state',
  onWorkspaceRouteChanged: 'workspace:route-changed',
  offWorkspaceRouteChanged: 'workspace:route-changed',
  onWorkspaceRouteRequested: 'workspace:route-requested',
  offWorkspaceRouteRequested: 'workspace:route-requested',
  onWorkspaceCloseActiveTab: 'workspace:close-active-tab',
  offWorkspaceCloseActiveTab: 'workspace:close-active-tab',
  onLedgerUpdateStatus: 'updates:status',
  offLedgerUpdateStatus: 'updates:status',
} as const;

const ledgerEvents = Object.fromEntries(
  Object.entries(ledgerEventChannels).map(([name, channel]) => [
    name,
    (listener: Function) =>
      name.startsWith('on')
...
const ledgerCommands = {
  calendarFollowUpCreated: (payload?: unknown) =>
    ipcRenderer.send('calendar:follow-up-created', payload),
  ...
  dashboardTodayTaskCreated: (payload?: unknown) =>
    ipcRenderer.send('dashboard:today-task-created', payload),
  dashboardTodayTaskDeleted: (payload?: unknown) =>
    ipcRenderer.send('dashboard:today-task-deleted', payload),
  ...
  notesSmartLinksUpdated: (payload?: unknown) =>
    ipcRenderer.send('notes:smart-links-updated', payload),
  notificationsRefresh: () => ipcRenderer.send('notifications:refresh'),
  notificationsSetSession: (payload?: unknown) =>
    ipcRenderer.send('notifications:set-session', payload),
  ...
  spellcheckAutocorrectNote: (payload: unknown) =>
    ipcRenderer.invoke('spellcheck:autocorrect-note', payload),
  spellcheckSuggestions: (payload: unknown) =>
    ipcRenderer.invoke('spellcheck:suggestions', payload),
...
contextBridge.exposeInMainWorld('appleCalendar', {
  status() {
    return ipcRenderer.invoke('apple-calendar:status');
  },
  requestAccess() {
    return ipcRenderer.invoke('apple-calendar:request-access');
  },
  listCalendars() {
    return ipcRenderer.invoke('apple-calendar:list-calendars');
  },
  events(payload: { start: string; end: string; calendarIds: string[] }) {
    return ipcRenderer.invoke('apple-calendar:events', payload);
  },
  refreshRange(payload: { start: string; end: string; calendarIds: string[] }) {
    return ipcRenderer.invoke('apple-calendar:refresh-range', payload);
  },
  getEvent(payload: { eventId: string }) {
    return ipcRenderer.invoke('apple-calendar:get-event', payload);
  },
  connectionStatus() {
    return ipcRenderer.invoke('apple-calendar:connection-status');
  },
  writableCalendars() {
    return ipcRenderer.invoke('apple-calendar:writable-calendars');
  },
  createEvent(payload: Record<string, unknown>) {
    return ipcRenderer.invoke('apple-calendar:create-event', payload);
  },
  updateEvent(payload: Record<string, unknown>) {
    return ipcRenderer.invoke('apple-calendar:update-event', payload);
  },
  deleteEvent(payload: { eventId: string; span?: 'thisEvent' | 'futureEvents' }) {
    return ipcRenderer.invoke('apple-calendar:delete-event', payload);
  },
  moveEvent(payload: { eventId: string; calendarId: string; span?: 'thisEvent' | 'futureEvents' }) {
    return ipcRenderer.invoke('apple-calendar:move-event', payload);
  },
  openSystemSettings() {
    return ipcRenderer.invoke('apple-calendar:open-system-settings');
  },
...
contextBridge.exposeInMainWorld('appleReminders', {
  getPermissionStatus() {
    return ipcRenderer.invoke('apple-reminders:permission-status');
  },
  getConnectionStatus() {
    return ipcRenderer.invoke('apple-reminders:connection-status');
  },
  requestAccess() {
    return ipcRenderer.invoke('apple-reminders:request-access');
  },
  getLists() {
    return ipcRenderer.invoke('apple-reminders:get-lists');
  },
  getWritableLists() {
    return ipcRenderer.invoke('apple-reminders:get-writable-lists');
  },
  getReminder(payload: { reminderId: string }) {
    return ipcRenderer.invoke('apple-reminders:get-reminder', payload);
  },
  createReminder(payload: Record<string, unknown>) {
    return ipcRenderer.invoke('apple-reminders:create-reminder', payload);
  },
  updateReminder(payload: Record<string, unknown>) {
    return ipcRenderer.invoke('apple-reminders:update-reminder', payload);
  },
  setCompleted(payload: { reminderId: string; completed: boolean; listId?: string }) {
    return ipcRenderer.invoke('apple-reminders:set-completed', payload);
  },
  moveReminder(payload: { reminderId: string; listId: string }) {
    return ipcRenderer.invoke('apple-reminders:move-reminder', payload);
  },
  deleteReminder(payload: { reminderId: string }) {
    return ipcRenderer.invoke('apple-reminders:delete-reminder', payload);
  },
  fetchReminders(payload: { start: string; end: string; listIds: string[] }) {
    return ipcRenderer.invoke('apple-reminders:fetch-reminders', payload);
  },
  refresh(payload: { start: string; end: string; listIds: string[] }) {
    return ipcRenderer.invoke('apple-reminders:refresh', payload);
  },
  disconnect() {
    return ipcRenderer.invoke('apple-reminders:disconnect');
  },
  openSystemSettings() {
    return ipcRenderer.invoke('apple-reminders:open-system-settings');
  },
});

contextBridge.exposeInMainWorld('meetingAudio', {
  permissions() {
    return ipcRenderer.invoke('meeting-audio:permissions');
  },
  requestPermissions() {
    return ipcRenderer.invoke('meeting-audio:request-permissions');
  },
  openSystemSettings(area: 'microphone' | 'screen-recording') {
    return ipcRenderer.invoke('meeting-audio:open-system-settings', area);
  },
  status() {
    return ipcRenderer.invoke('meeting-audio:status');
  },
  storagePath() {
    return ipcRenderer.invoke('meeting-audio:storage-path');
  },
  openStoragePath() {
    return ipcRenderer.invoke('meeting-audio:open-storage-path');
  },
  devices() {
    return ipcRenderer.invoke('meeting-audio:devices');
  },
  recoveries() {
    return ipcRenderer.invoke('meeting-audio:recoveries');
  },
  inspect(sessionId?: string) {
    return ipcRenderer.invoke('meeting-audio:inspect', sessionId);
  },
  recover(payload: { sessionId: string; noteId: string; workspaceId: string }) {
    return ipcRenderer.invoke('meeting-audio:recover', payload);
  },
  discardRecovery(sessionId: string) {
    return ipcRenderer.invoke('meeting-audio:discard-recovery', sessionId);
  },
  start(payload: {
    noteId: string;
    workspaceId: string;
  ...
  pause() {
    return ipcRenderer.invoke('meeting-audio:pause');
  },
  resume() {
    return ipcRenderer.invoke('meeting-audio:resume');
  },
  stop() {
    return ipcRenderer.invoke('meeting-audio:stop');
  },
  ...
  onLevel(
    ...
    const wrapped = (
      ...
      payload: { source: 'user_microphone' | 'system_audio'; level: number }
    ) => listener(payload);
  ...
  onError(
    ...
    const wrapped = (
      _event: Electron.IpcRendererEvent,
      payload: { source: 'user_microphone' | 'system_audio'; error: string }
    ) => listener(payload);
...
contextBridge.exposeInMainWorld('speakerTags', {
  ...
  setup() {
    return ipcRenderer.invoke('speaker-tags:setup');
  },
  start() {
    return ipcRenderer.invoke('speaker-tags:start');
  },
...
contextBridge.exposeInMainWorld('meetingAutoStop', {
  keepRecording() {
    return ipcRenderer.invoke('meeting-auto-stop:keep-recording');
  },
  signalCallEnded(noteId: string) {
    return ipcRenderer.invoke('meeting-auto-stop:call-ended', { noteId });
  },
  signalNewMeeting(payload: { noteId: string; title?: string }) {
    return ipcRenderer.invoke('meeting-auto-stop:new-meeting', payload);
  },
  onGrace(listener: (event: { active: boolean; noteId: string; reason?: string }) => void) {
    const wrapped = (
      _event: Electron.IpcRendererEvent,
      payload: { active: boolean; noteId: string; reason?: string }
    ) => listener(payload);
  ...
  onStopRequested(listener: (event: { noteId: string; reason: string }) => void) {
    const wrapped = (
      _event: Electron.IpcRendererEvent,
      payload: { noteId: string; reason: string }
    ) => listener(payload);
  ...
  onCompleted(listener: (event: { noteId: string; sessionId: string }) => void) {
    const wrapped = (
      _event: Electron.IpcRendererEvent,
      payload: { noteId: string; sessionId: string }
    ) => listener(payload);
...
contextBridge.exposeInMainWorld('meetingTranscription', {
  ...
  downloadModel() {
    return ipcRenderer.invoke('meeting-transcription:download-model');
  },
  cancelModelDownload() {
    return ipcRenderer.invoke('meeting-transcription:cancel-model-download');
  },
  deleteModel() {
    return ipcRenderer.invoke('meeting-transcription:delete-model');
  },
  prepare(payload: { sessionId: string; noteId: string; workspaceId: string }) {
    return ipcRenderer.invoke('meeting-transcription:prepare', payload);
  },
  status(jobId?: string) {
    return ipcRenderer.invoke('meeting-transcription:status', jobId);
  },
  start(payload: { sessionId: string; noteId: string; workspaceId: string; force?: boolean }) {
    return ipcRenderer.invoke('meeting-transcription:start', payload);
  },
  cancel(jobId: string) {
    return ipcRenderer.invoke('meeting-transcription:cancel', jobId);
  },
  results(jobId: string) {
    return ipcRenderer.invoke('meeting-transcription:results', jobId);
  },
  complete(payload: { jobId: string; retention: 'delete_after_transcription' | 'retain' }) {
    return ipcRenderer.invoke('meeting-transcription:complete', payload);
  },
  fail(payload: { jobId: string; error: string }) {
    return ipcRenderer.invoke('meeting-transcription:fail', payload);
  },
...
contextBridge.exposeInMainWorld('askLedger', {
  generateOverviewFocus(snapshot: unknown, options?: { previousResult?: unknown }) {
    ...
    }) as Promise<{ insights: unknown[] }>;
  },
  generateProjectLens(payload: unknown) {
    return ipcRenderer.invoke('project-lens:generate', payload) as Promise<unknown>;
  },
  generateProjectLensAction(payload: unknown) {
    return ipcRenderer.invoke('project-lens:action', payload) as Promise<unknown>;
  },
  generateMeetingRecap(payload: unknown) {
    return ipcRenderer.invoke('meeting-recap:generate', payload) as Promise<unknown>;
  },
  generateMeetingPeople(payload: unknown) {
    return ipcRenderer.invoke('meeting-people:suggest', payload) as Promise<unknown>;
  },
  generateMeetingPrep(payload: unknown) {
    return ipcRenderer.invoke('meeting-prep:generate', payload) as Promise<unknown>;
  },
  listSkills() {
    return ipcRenderer.invoke('ask-ledger:list-skills') as Promise<unknown[]>;
  },
  selectAttachments(payload: {
    ...
    return ipcRenderer.invoke('ask-ledger:select-attachments', payload);
  },
  openAttachment(attachmentId: string) {
    return ipcRenderer.invoke('ask-ledger:open-attachment', attachmentId) as Promise<{
      ok: boolean;
      error?: string;
    }>;
  },
  removeAttachments(payload: { conversationId: string; attachmentIds: string[] }) {
    return ipcRenderer.invoke('ask-ledger:remove-attachments', payload) as Promise<{ ok: boolean }>;
  },
  localAIStatus() {
  ...
  setAIProviderKey(payload: { provider: 'openai' | 'anthropic' | 'google' | 'perplexity' | 'kimi' | 'deepseek'; apiKey: string }) {
    return ipcRenderer.invoke('ask-ledger:ai-provider-set-key', payload);
  },
  removeAIProviderKey(provider: 'openai' | 'anthropic' | 'google' | 'perplexity' | 'kimi' | 'deepseek') {
    return ipcRenderer.invoke('ask-ledger:ai-provider-remove-key', provider);
  },
  testAIProvider(provider: 'openai' | 'anthropic' | 'google' | 'perplexity' | 'kimi' | 'deepseek') {
    return ipcRenderer.invoke('ask-ledger:ai-provider-test', provider);
  },
  listAIProviderModels(provider: 'openai' | 'anthropic' | 'google' | 'perplexity' | 'kimi' | 'deepseek') {
    return ipcRenderer.invoke('ask-ledger:ai-provider-models', provider);
  },
  getSelectedAIProviderModel(provider: 'openai' | 'anthropic' | 'google' | 'perplexity' | 'kimi' | 'deepseek') {
    return ipcRenderer.invoke('ask-ledger:ai-provider-selected-model', provider);
  },
  setSelectedAIProviderModel(payload: { provider: 'openai' | 'anthropic' | 'google' | 'perplexity' | 'kimi' | 'deepseek'; model: string }) {
  ...
  setSelectedAIProvider(provider: 'local' | 'openai' | 'anthropic' | 'google' | 'perplexity' | 'kimi' | 'deepseek') {
    return ipcRenderer.invoke('ask-ledger:ai-provider-select', provider);
  },
  onAIProviderState(listener: (state: { provider: 'local' | 'openai' | 'anthropic' | 'google' | 'perplexity' | 'kimi' | 'deepseek'; cloudConsent: boolean; model: string | null }) => void) {
    const handler = (_event: unknown, state: { provider: 'local' | 'openai' | 'anthropic' | 'google' | 'perplexity' | 'kimi' | 'deepseek'; cloudConsent: boolean; model: string | null }) => listener(state);
  ...
  localAICapability() {
    return ipcRenderer.invoke('ask-ledger:local-ai-capability');
  },
  acknowledgeLocalAITier(tier: 'fast' | 'balanced') {
    return ipcRenderer.invoke('ask-ledger:local-ai-acknowledge-tier', tier);
  },
  getGenerationModels() {
    return ipcRenderer.invoke('ask-ledger:generation-models');
  },
  getAvailableGenerationModels() {
    return ipcRenderer.invoke('ask-ledger:generation-models');
  },
  getSelectedGenerationTier() {
    return ipcRenderer.invoke('ask-ledger:selected-generation-tier');
  },
  setSelectedGenerationTier(tier: 'fast' | 'balanced') {
    return ipcRenderer.invoke('ask-ledger:set-selected-generation-tier', tier);
  },
  switchGenerationTier(tier: 'fast' | 'balanced') {
    return ipcRenderer.invoke('ask-ledger:switch-generation-tier', tier);
  },
  getGenerationRuntimeState() {
    return ipcRenderer.invoke('ask-ledger:generation-runtime-state');
  },
  getGenerationModelStatus(modelId: string) {
    return ipcRenderer.invoke('ask-ledger:generation-model-status', modelId);
  },
  downloadGenerationModel(modelId: string) {
    return ipcRenderer.invoke('ask-ledger:generation-model-download', modelId);
  },
  cancelGenerationModelDownload(modelId: string) {
    return ipcRenderer.invoke('ask-ledger:generation-model-cancel-download', modelId);
  },
  removeGenerationModel(modelId: string) {
    return ipcRenderer.invoke('ask-ledger:generation-model-remove', modelId);
  },
  downloadLocalAI(role: 'generation' | 'embedding') {
    return ipcRenderer.invoke('ask-ledger:local-ai-download', role);
  },
  cancelLocalAIDownload(role: 'generation' | 'embedding') {
    return ipcRenderer.invoke('ask-ledger:local-ai-cancel-download', role);
  },
  removeLocalAI(role: 'generation' | 'embedding') {
    return ipcRenderer.invoke('ask-ledger:local-ai-remove', role);
  },
  start(payload: {
    question: string;
    workspaceId: string;
...
contextBridge.exposeInMainWorld('desktopWindow', {
  ...
  onMeetingIndicatorState(
    ...
    const wrapped = (
      _event: Electron.IpcRendererEvent,
      state: { recording: boolean; paused: boolean; activity: 'silent' | 'low' | 'medium' | 'high' }
    ) => listener(state);
