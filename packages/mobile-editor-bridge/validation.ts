...
export function parseNativeEditorCommand(value: unknown): NativeEditorCommand | null {
  ...
  if (value.type === 'INSERT_LINK_TEXT' && isNonEmptyString(value.url) && value.url.length <= 2000 && isNonEmptyString(value.text) && value.text.length <= 240) return value as NativeEditorCommand;
...
export function parseEditorNativeEvent(value: unknown): EditorNativeEvent | null {
  ...
  if (value.type === 'COPY_IMAGE_REQUEST' && isNonEmptyString(value.noteId) && isGeneration(value.generation) && isNonEmptyString(value.src) && value.src.length <= 12000) return value as EditorNativeEvent;
