...
export function parseNativeEditorCommand(value: unknown): NativeEditorCommand | null {
  ...
  if (value.type === 'LOAD_DOCUMENT' && isNonEmptyString(value.noteId) && isNonEmptyString(value.requestId) && typeof value.html === 'string' && value.html.length <= MAX_DOCUMENT_HTML_LENGTH && (value.readOnly === undefined || typeof value.readOnly === 'boolean')) return value as NativeEditorCommand;
  if (value.type === 'REQUEST_EXPORT' && isNonEmptyString(value.noteId) && isNonEmptyString(value.requestId)) return value as NativeEditorCommand;
  if (value.type === 'REQUEST_SELECTION' && isNonEmptyString(value.noteId) && isNonEmptyString(value.requestId)) return value as NativeEditorCommand;
...
export function parseEditorNativeEvent(value: unknown): EditorNativeEvent | null {
  if (!isRecord(value) || typeof value.type !== 'string') return null;
  if (value.type === 'READY' || value.type === 'FOCUSED' || value.type === 'BLURRED') return value as EditorNativeEvent;
  if (value.type === 'DOCUMENT_LOADED' && isNonEmptyString(value.noteId) && isNonEmptyString(value.requestId)) return value as EditorNativeEvent;
  if (value.type === 'DIRTY_STATE_CHANGED' && isNonEmptyString(value.noteId) && typeof value.dirty === 'boolean') return value as EditorNativeEvent;
  if (value.type === 'SELECTION_STATE_CHANGED' && isNonEmptyString(value.noteId) && isRecord(value.selection) && typeof value.selection.bold === 'boolean' && typeof value.selection.italic === 'boolean' && typeof value.selection.underline === 'boolean' && typeof value.selection.canUndo === 'boolean' && typeof value.selection.canRedo === 'boolean' && ['paragraph', 'h1', 'h2', 'h3'].includes(String(value.selection.blockType)) && (value.selection.listType === undefined || ['bullet', 'number', 'check'].includes(String(value.selection.listType))) && (value.selection.linkUrl === undefined || typeof value.selection.linkUrl === 'string')) return value as EditorNativeEvent;
  if (value.type === 'DOCUMENT_EXPORTED' && isNonEmptyString(value.noteId) && isNonEmptyString(value.requestId) && typeof value.html === 'string' && typeof value.plainText === 'string') return value as EditorNativeEvent;
  if (value.type === 'SELECTION_RESULT' && isNonEmptyString(value.noteId) && isNonEmptyString(value.requestId) && typeof value.plainText === 'string' && (value.html === undefined || typeof value.html === 'string')) return value as EditorNativeEvent;
  if (value.type === 'ERROR' && typeof value.message === 'string') return value as EditorNativeEvent;
  return null;
}
