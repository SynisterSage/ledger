...
const PRESERVED_TAGS = ['aside', 'figure', 'img', 'hr', 'section', 'div', 'ul'];
...
export class LedgerPreservationNode extends ElementNode {
  ...
  static importDOM() {
    return Object.fromEntries(PRESERVED_TAGS.map((tag) => [tag, (element: HTMLElement) => hasLedgerMetadata(element) || tag === 'img' || tag === 'hr' ? { conversion: () => ({ node: new LedgerPreservationNode(element.outerHTML, tag) }), priority: 4 } : null]));
  }
  createDOM() {
