...
type Props = { showToolbar?: boolean; showStatus?: boolean; workspaceId?: string; noteId?: string; onEvent?: (event: EditorNativeEvent) => void; onEmbeddedError?: (message: string) => void; onStage?: (stage: MobileEditorStage, detail?: string) => void; onLedgerLink?: (url: string) => void };
...
export const MobileLexicalEditor = forwardRef<MobileLexicalEditorHandle, Props>(function MobileLexicalEditor({ showToolbar = true, showStatus = true, workspaceId, noteId: propNoteId, onEvent, onEmbeddedError, onStage, onLedgerLink }, ref) {
  ...
  <WebView key={webViewGeneration} ref={webViewRef} source={{ html: MOBILE_EDITOR_HTML, baseUrl: 'about:blank' }} injectedJavaScriptBeforeContentLoaded={`window.__ledgerEditorGeneration = ${webViewGeneration}; true;`} onMessage={onMessage} onLoadStart={onLoadStart} onLoadEnd={() => onStage?.('html-load-end')} onError={(event) => { const message = event.nativeEvent.description || 'The embedded editor was interrupted.'; setError(message); onStage?.('webview-error', message); onEmbeddedError?.(message); onEvent?.({ type: 'EDITOR_ERROR', generation: generationRef.current, code: 'WEBVIEW_LOAD_FAILED', message }); }} onHttpError={(event) => { const message = `Editor asset request failed (${event.nativeEvent.statusCode}).`; onStage?.('http-error', message); setError(message); onEmbeddedError?.(message); }} originWhitelist={['about:blank']} javaScriptEnabled domStorageEnabled={false} allowFileAccess allowUniversalAccessFromFileURLs={false} scrollEnabled automaticallyAdjustContentInsets={false} keyboardDisplayRequiresUserAction={false} setBuiltInZoomControls={false} scalesPageToFit={false} style={styles.webview} onShouldStartLoadWithRequest={(request) => { if (isLedgerLink(request.url)) { onLedgerLink?.(request.url); return false; } if (/^https?:\/\//i.test(request.url)) { openExternalLink(request.url); return false; } return request.url === 'about:blank'; }} />
    {toolbarVisible ? <ScrollView horizontal showsHorizontalScrollIndicator={false} keyboardShouldPersistTaps="handled" style={[styles.toolbar, { borderTopColor: theme.colors.borderSubtle, backgroundColor: theme.colors.background }]} contentContainerStyle={styles.toolbarContent}><ToolbarButton label="Undo" icon={{ ios: 'arrow.uturn.backward', android: 'undo', web: 'undo' }} disabled={!selection.canUndo} onPress={() => command({ type: 'UNDO' })} /><ToolbarButton label="Redo" icon={{ ios: 'arrow.uturn.forward', android: 'redo', web: 'redo' }} disabled={!selection.canRedo} onPress={() => command({ type: 'REDO' })} /><ToolbarButton label="Text style" icon={{ ios: 'textformat', android: 'text_format', web: 'text_format' }} active={selection.blockType !== 'paragraph'} onPress={() => setBlockSheetOpen(true)} /><ToolbarButton label="Bold" icon={{ ios: 'bold', android: 'format_bold', web: 'format_bold' }} active={selection.bold} onPress={() => command({ type: 'TOGGLE_FORMAT', format: 'bold' })} /><ToolbarButton label="Italic" icon={{ ios: 'italic', android: 'format_italic', web: 'format_italic' }} active={selection.italic} onPress={() => command({ type: 'TOGGLE_FORMAT', format: 'italic' })} /><ToolbarButton label="Underline" icon={{ ios: 'underline', android: 'format_underlined', web: 'format_underlined' }} active={selection.underline} onPress={() => command({ type: 'TOGGLE_FORMAT', format: 'underline' })} /><ToolbarButton label="List" icon={{ ios: 'list.bullet', android: 'format_list_bulleted', web: 'format_list_bulleted' }} active={selection.listType === 'bullet' || selection.listType === 'number'} onPress={() => setListSheetOpen(true)} /><ToolbarButton label="Checklist" icon={{ ios: 'checklist', android: 'checklist', web: 'checklist' }} active={selection.listType === 'check'} onPress={() => command({ type: 'TOGGLE_LIST', list: 'check' })} /><ToolbarButton label="Link" icon={{ ios: 'link', android: 'link', web: 'link' }} active={Boolean(selection.linkUrl)} onPress={openLinkSheet} /><ToolbarButton label="Ledger" onPress={() => { requestSelection(identityRef.current?.noteId); }} /><ToolbarButton label="Insert" icon={{ ios: 'plus', android: 'add', web: 'add' }} onPress={() => setInsertSheetOpen(true)} /></ScrollView> : null}
...
const styles = StyleSheet.create({ container: { flex: 1 }, toolbar: { maxHeight: 52, borderTopWidth: StyleSheet.hairlineWidth }, toolbarContent: { alignItems: 'center', paddingHorizontal: 8, gap: 1 }, status: { minHeight: 28, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 8 }, webview: { flex: 1, backgroundColor: 'transparent' }, sheetCard: { overflow: 'hidden', paddingHorizontal: 16, paddingVertical: 6 }, sheetRow: { minHeight: 56, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, linkInput: { minHeight: 48, borderBottomWidth: 0, fontSize: 16, paddingHorizontal: 0, paddingVertical: 8 }, sheetActions: { flexDirection: 'row', gap: 20, paddingTop: 16, flexWrap: 'wrap' }, toolbarButton: { minHeight: 44, minWidth: 44, paddingHorizontal: 8, borderRadius: 8, alignItems: 'center', justifyContent: 'center' } });
