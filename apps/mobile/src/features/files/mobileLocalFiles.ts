...
const root = (userId: string, workspaceId: string) => `${FileSystem.documentDirectory ?? FileSystem.cacheDirectory ?? ''}ledger-local-files/${safePathPart(userId)}/${safePathPart(workspaceId)}/`;
const manifestPath = (userId: string, workspaceId: string) => `${root(userId, workspaceId)}manifest.json`;
...
export async function importMobileLocalFile(asset: { uri: string; name: string; mimeType?: string | null; size?: number | null }, userId: string, workspaceId: string) {
  ...
  const file: MobileLocalFile = { id, name: asset.name, mimeType: asset.mimeType, sizeBytes: asset.size, uri, createdAt: new Date().toISOString() };
...
export async function removeMobileLocalFile(id: string, userId: string, workspaceId: string) {
  ...
  await writeManifest(files.filter((item) => item.id !== id), userId, workspaceId);
  return true;
}
