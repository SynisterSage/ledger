...
function OptionRow({ label, selected, onPress }: { label: string; selected: boolean; onPress: () => void }) {
  ...
  return <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: selected }} onPress={onPress} style={({ pressed }) => [styles.option, { borderBottomColor: theme.colors.borderSubtle, opacity: pressed ? 0.68 : 1 }]}><AppText variant="body">{label}</AppText>{selected ? <AppText variant="body" style={{ color: theme.colors.accent }}>✓</AppText> : null}</Pressable>;
}

function Group({ title, children }: { title: string; children: ReactNode }) {
  return <View style={styles.group}><AppText variant="label" style={styles.groupLabel}>{title}</AppText>{children}</View>;
}
...
export function NoteFilterSheet({ visible, filters, sections, hasUnsorted, onChange, onReset, onClose }: NoteFilterSheetProps) {
  ...
  return <AppBottomSheet visible={visible} onClose={onClose} snapPoints={['72%', '90%']} initialSnapPointIndex={0} title={<View><AppText variant="sectionTitle">Filter notes</AppText>{countActiveNoteFilters(filters) ? <AppText variant="caption">{countActiveNoteFilters(filters)} active</AppText> : null}</View>} headerAccessory={<Pressable onPress={onReset} hitSlop={8}><AppText variant="caption" style={{ color: theme.colors.accent }}>Reset</AppText></Pressable>}>
...
const styles = StyleSheet.create({
  ...
  option: { minHeight: 44, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: StyleSheet.hairlineWidth },
});
