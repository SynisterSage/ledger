...
export function NoteActionSheet({ visible, note, onClose, onOpen, onTogglePin, onMove, onDuplicate, onChild, onProjects, onDelete, onVersionHistory, pinned, permissions }: { visible: boolean; note: MobileNoteSummary | null; onClose: () => void; onOpen: () => void; onTogglePin: () => void; onMove: () => void; onDuplicate: () => void; onChild: () => void; onProjects: () => void; onDelete: () => void; onVersionHistory?: () => void; pinned: boolean; permissions?: MobileNotePermissions }) {
  ...
  const actions: MobileActionSheetAction[] = [
    ...
    ...(onVersionHistory ? [{ id: 'version-history', label: 'Version history', perform: () => { onClose(); onVersionHistory(); } }] : []),
...
export function NoteMoveSheet({ visible, note, sections, notes = [], onClose, onMove, onParentMove }: { visible: boolean; note: MobileNoteSummary | null; sections: MobileNoteSection[]; notes?: MobileNoteSummary[]; onClose: () => void; onMove: (sectionId: string | null) => void; onParentMove?: (parentId: string | null) => void }) {
  ...
  return <AppBottomSheet visible={visible} onClose={onClose} title={<AppText variant="sectionTitle">Move note</AppText>} snapPoints={['58%', '84%']} initialSnapPointIndex={0}><View style={[styles.card, { backgroundColor: theme.colors.surfaceMuted }]}><ScrollView><Pressable onPress={() => onMove(null)} style={styles.actionRow}><AppText variant="body">Unsorted</AppText>{!note?.section_id ? <AppText variant="caption" style={{ color: theme.colors.accent }}>✓</AppText> : null}</Pressable>{roots.map((section) => <SectionChoice key={section.id} section={section} sections={sections} selected={note?.section_id === section.id} onMove={onMove} />)}{onParentMove ? <><AppText variant="label" style={styles.groupLabel}>PARENT NOTE</AppText><Pressable onPress={() => onParentMove(null)} style={styles.actionRow}><AppText variant="body">Root level</AppText>{!note?.parent_id ? <AppText variant="caption" style={{ color: theme.colors.accent }}>✓</AppText> : null}</Pressable>{notes.filter((item) => !invalidParents.has(item.id)).slice(0, 30).map((item) => <Pressable key={item.id} onPress={() => onParentMove(item.id)} style={[styles.actionRow, styles.nestedRow]}><AppText variant="body" numberOfLines={1}>{item.title || 'Untitled'}</AppText>{note?.parent_id === item.id ? <AppText variant="caption" style={{ color: theme.colors.accent }}>✓</AppText> : null}</Pressable>)}</> : null}</ScrollView></View></AppBottomSheet>;
}
...
export function SectionActionSheet({ visible, workspaceId, section, onClose, onOpen, onNewNote, onNewSubsection, onChanged }: { visible: boolean; workspaceId: string; section: MobileNoteSection | null; onClose: () => void; onOpen: () => void; onNewNote: () => void; onNewSubsection: () => void; onChanged: () => void }) {
  ...
  if (renaming) return <AppBottomSheet visible={visible} onClose={onClose} title={<AppText variant="sectionTitle">Rename section</AppText>} snapPoints={['55%', '72%']} initialSnapPointIndex={0}><View style={[styles.inputCard, { backgroundColor: theme.colors.surfaceMuted }]}><TextInput autoFocus value={name} onChangeText={setName} style={[styles.input, styles.cardInput, { color: theme.colors.textPrimary }]} /></View><Pressable disabled={busy} onPress={() => void rename()} style={styles.submit}><AppText variant="button" style={{ color: '#fff' }}>{busy ? 'Saving…' : 'Rename section'}</AppText></Pressable></AppBottomSheet>;
  const actions: MobileActionSheetAction[] = [
    { id: 'open', label: 'Open', perform: () => { onClose(); onOpen(); } },
...
const styles = StyleSheet.create({ card: { borderRadius: 18, paddingHorizontal: 16, paddingVertical: 6 }, actionRow: { minHeight: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, actionMenuRow: { minHeight: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, nestedRow: { paddingLeft: 22 }, choiceCopy: { flex: 1, gap: 2 }, groupLabel: { marginTop: 16, marginBottom: 4 }, destructive: { color: '#B42318' }, search: { minHeight: 42, borderWidth: 1, borderRadius: 10, paddingHorizontal: 12, justifyContent: 'center', marginBottom: 10 }, inputCard: { borderRadius: 18, paddingHorizontal: 16, paddingVertical: 14 }, input: { minHeight: 46, borderBottomWidth: 1, fontSize: 16, marginVertical: 12 }, cardInput: { borderBottomWidth: 0, marginVertical: 0 }, submit: { alignItems: 'center', backgroundColor: '#FF5F40', borderRadius: 14, paddingVertical: 13, marginTop: 14 } });
