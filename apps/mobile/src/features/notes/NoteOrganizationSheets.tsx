...
export function NoteActionSheet({ visible, note, onClose, onOpen, onShare, onChangeType, onScanText, onTogglePin, onMove, onDuplicate, onChild, onProjects, onDelete, onVersionHistory, pinned, permissions }: { visible: boolean; note: MobileNoteSummary | null; onClose: () => void; onOpen: () => void; onShare?: () => void | Promise<void>; onChangeType?: () => void | Promise<void>; onScanText?: () => void | Promise<void>; onTogglePin: () => void; onMove: () => void; onDuplicate: () => void; onChild: () => void; onProjects: () => void; onDelete: () => void; onVersionHistory?: () => void; pinned: boolean; permissions?: MobileNotePermissions }) {
  ...
  const actions: MobileActionSheetAction[] = [
    ...
    ...(access.canDuplicate ? [{ id: 'duplicate', label: 'Duplicate', perform: () => afterClose(onDuplicate) }] : []),
    ...(access.canCreateChild ? [{ id: 'child', label: 'Create child note', perform: () => afterClose(onChild) }] : []),
    ...(access.canLinkProject ? [{ id: 'project', label: 'Link to project', perform: () => afterClose(onProjects) }] : []),
    ...(access.canDelete ? [{ id: 'delete', label: 'Delete', role: 'destructive' as const, perform: () => afterClose(onDelete) }] : []),
  ];
...
export function NoteProjectSheet({ visible, workspaceId, note, onClose, onChanged }: { visible: boolean; workspaceId: string; note: MobileNoteSummary | null; onClose: () => void; onChanged: () => void }) {
  ...
  const refreshLinks = async (noteId: string) => {
    const nextLinks = await getMobileWorkspaceNoteLinks(workspaceId, noteId);
    setLinks(nextLinks);
    return nextLinks;
  };
  ...
  if (!note) return null;
  const cardRadius = concentricRadius(theme.radius.sheet, theme.spacing.lg);
  return <AppBottomSheet visible={visible} onClose={onClose} title={<AppText variant="sectionTitle">Link to project</AppText>} headerAccessory={<Pressable accessibilityRole="button" accessibilityLabel="Done" onPress={onClose} hitSlop={8}><SymbolView name={{ ios: 'checkmark', android: 'check', web: 'check' }} size={22} tintColor={theme.colors.accent} /></Pressable>} snapPoints={['70%', '90%']} initialSnapPointIndex={0}><View style={[styles.search, { backgroundColor: theme.colors.surfaceMuted, borderRadius: cardRadius }]}><TextInput value={query} onChangeText={setQuery} placeholder="Search projects…" placeholderTextColor={theme.colors.placeholder} style={{ color: theme.colors.textPrimary, flex: 1 }} /></View><View style={[styles.card, { backgroundColor: theme.colors.surfaceMuted, borderRadius: cardRadius }]}>{filtered.length ? filtered.map((project) => { const linked = links.some((link) => link.project_id === project.id); return <Pressable key={project.id} disabled={busy === project.id} onPress={async () => { setBusy(project.id); try { if (linked) { await unlinkMobileNoteFromProject(workspaceId, project.id, note.id); await refreshLinks(note.id); } else { await linkMobileNoteToProject(workspaceId, project.id, note.id); await refreshLinks(note.id); } onChanged(); } catch (error) { Alert.alert('Could not update project link', error instanceof Error ? error.message : 'Please try again.'); } finally { setBusy(null); } }} style={({ pressed }) => [styles.actionRow, { opacity: busy === project.id ? 0.45 : pressed ? 0.68 : 1 }]}><AppText variant="body">{project.name}</AppText>{linked ? <SymbolView name={{ ios: 'checkmark', android: 'check', web: 'check' }} size={18} tintColor={theme.colors.accent} /> : null}</Pressable>; }) : <AppText variant="caption" style={styles.emptyProject}>No projects found.</AppText>}</View></AppBottomSheet>;
}
