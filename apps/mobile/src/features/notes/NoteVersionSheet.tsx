...
export function NoteVersionSheet({ visible, workspaceId, noteId, onClose, onRestored }: { visible: boolean; workspaceId: string; noteId: string; onClose: () => void; onRestored: (note: unknown) => void }) {
  ...
  return <AppBottomSheet visible={visible} onClose={onClose} title={<AppText variant="sectionTitle">Version history</AppText>} snapPoints={['68%', '88%']} initialSnapPointIndex={0}>{error ? <AppText variant="caption" style={{ color: theme.colors.danger }}>{error}</AppText> : null}<ScrollView>{versions.map((version) => <Pressable key={version.id} disabled={busy} onPress={() => restore(version)} style={[styles.row, { borderBottomColor: theme.colors.borderSubtle }]}><AppText variant="body">{version.reason === 'before_edit' ? 'Before edit' : version.reason === 'restore_before' ? 'Before restore' : version.reason === 'delete' ? 'Before delete' : 'Autosave checkpoint'}</AppText><AppText variant="caption">{version.created_at ? new Date(version.created_at).toLocaleString() : 'Unknown time'}</AppText></Pressable>)}{!versions.length && !error ? <AppText variant="caption">No saved versions yet.</AppText> : null}</ScrollView></AppBottomSheet>;
}

const styles = StyleSheet.create({ row: { minHeight: 54, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: StyleSheet.hairlineWidth } });
