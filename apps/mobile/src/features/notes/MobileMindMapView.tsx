...
function NodeActions({ controller, onRename, onMove, editable = true }: { controller: Controller; onRename: () => void; onMove: () => void; editable?: boolean }) {
  const theme = useLedgerTheme();
  const { selected } = controller;
  if (!editable) return null;
  return <View style={[styles.actionBar, { backgroundColor: theme.colors.surface, borderColor: theme.colors.borderSubtle }]}><Pressable accessibilityRole="button" accessibilityLabel="Add child node" onPress={() => controller.addNode(false)} style={styles.actionButton}><AppText variant="caption">+ Child</AppText></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Add sibling node" onPress={() => controller.addNode(true)} disabled={!selected || selected.id === controller.parsed.rootId} style={styles.actionButton}><AppText variant="caption">+ Sibling</AppText></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Rename node" onPress={onRename} style={styles.actionButton}><AppText variant="caption">Rename</AppText></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Move node" onPress={onMove} style={styles.actionButton}><AppText variant="caption">Move</AppText></Pressable></View>;
}
...
export function MobileMindMapView({ structure, view, onChange, title = 'Central Idea', hideControls = false, editable = true }: { structure: unknown; view: 'map' | 'outline'; onChange: (next: unknown) => void; title?: string; hideControls?: boolean; editable?: boolean }) {
  const theme = useLedgerTheme();
  ...
  if (view === 'outline') return <View style={styles.mapRoot}><ScrollView contentContainerStyle={styles.outline}>{controller.parsed.error ? <InvalidState title={title} onCreateRoot={controller.createRoot} /> : outlineRows(controller.parsed.rootId, controller, theme, 0, setActionOpen)}</ScrollView><NodeSheets controller={controller} editable={editable} actionOpen={actionOpen} setActionOpen={setActionOpen} renameOpen={renameOpen} setRenameOpen={setRenameOpen} moveOpen={moveOpen} setMoveOpen={setMoveOpen} rename={rename} setRename={setRename} /></View>;
...
function NodeSheets({ controller, editable = true, actionOpen, setActionOpen, renameOpen, setRenameOpen, moveOpen, setMoveOpen, rename, setRename }: { controller: Controller; editable?: boolean; actionOpen: boolean; setActionOpen: (value: boolean) => void; renameOpen: boolean; setRenameOpen: (value: boolean) => void; moveOpen: boolean; setMoveOpen: (value: boolean) => void; rename: string; setRename: (value: string) => void }) { const theme = useLedgerTheme(); const selected = controller.selected; const cardRadius = concentricRadius(theme.radius.sheet, theme.spacing.lg); const done = (onPress: () => void, label = 'Done') => <Pressable accessibilityRole="button" accessibilityLabel={label} onPress={onPress} hitSlop={8}><SymbolView name={{ ios: 'checkmark', android: 'check', web: 'check' }} size={22} tintColor={theme.colors.accent} /></Pressable>; const afterActionSheetClose = (action: () => void) => { setActionOpen(false); setTimeout(action, 320); }; const moveTargets = selected ? Object.values(controller.parsed.nodes).filter((node) => node.id !== selected.id && !controller.descendants(selected.id).has(node.id)) : []; if (!editable) return null; return <><AppBottomSheet visible={actionOpen} onClose={() => setActionOpen(false)} title={<AppText variant="sectionTitle">{selected?.label ?? 'Map node'}</AppText>} headerAccessory={done(() => setActionOpen(false))} snapPoints={['58%', '82%']} initialSnapPointIndex={0}><View style={[styles.sheetCard, { backgroundColor: theme.colors.surfaceMuted, borderRadius: cardRadius }]}><Pressable style={styles.actionRow} onPress={() => afterActionSheetClose(() => controller.addNode(false))}><AppText variant="body">Add child</AppText></Pressable>{selected?.id !== controller.parsed.rootId ? <Pressable style={styles.actionRow} onPress={() => afterActionSheetClose(() => controller.addNode(true))}><AppText variant="body">Add sibling</AppText></Pressable> : null}<Pressable style={styles.actionRow} onPress={() => afterActionSheetClose(() => { setRename(selected?.label ?? ''); setRenameOpen(true); })}><AppText variant="body">Rename</AppText></Pressable><Pressable style={styles.actionRow} onPress={() => afterActionSheetClose(controller.toggleComplete)}><AppText variant="body">{selected?.completed ? 'Mark incomplete' : 'Complete'}</AppText></Pressable><Pressable style={styles.actionRow} onPress={() => afterActionSheetClose(controller.toggleCollapse)}><AppText variant="body">{selected?.collapsed ? 'Expand branch' : 'Collapse branch'}</AppText></Pressable>{selected?.id !== controller.parsed.rootId ? <Pressable style={styles.actionRow} onPress={() => afterActionSheetClose(() => setMoveOpen(true))}><AppText variant="body">Move</AppText></Pressable> : null}{selected?.id !== controller.parsed.rootId ? <Pressable style={styles.actionRow} onPress={() => afterActionSheetClose(() => Alert.alert('Delete branch?', 'This removes the selected node and its descendants from the map.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Delete', style: 'destructive', onPress: controller.removeNode }]))}><AppText variant="body" style={{ color: theme.colors.danger }}>Delete</AppText></Pressable> : null}</View></AppBottomSheet><AppBottomSheet visible={moveOpen} onClose={() => setMoveOpen(false)} title={<AppText variant="sectionTitle">Move node</AppText>} headerAccessory={done(() => setMoveOpen(false))} snapPoints={['58%', '82%']} initialSnapPointIndex={0}><View style={[styles.sheetCard, { backgroundColor: theme.colors.surfaceMuted, borderRadius: cardRadius }]}>{moveTargets.length ? moveTargets.map((node) => <Pressable key={node.id} style={styles.actionRow} onPress={() => { controller.moveTo(node.id); setMoveOpen(false); }}><AppText variant="body">{node.label}</AppText></Pressable>) : <AppText variant="caption" style={styles.emptySheetState}>No valid parent nodes available.</AppText>}</View></AppBottomSheet><AppBottomSheet visible={renameOpen} onClose={() => setRenameOpen(false)} title={<AppText variant="sectionTitle">Rename node</AppText>} headerAccessory={done(() => { controller.renameNode(rename); setRenameOpen(false); }, 'Save node name')} snapPoints={['34%', '48%']} initialSnapPointIndex={0}><View style={[styles.inputCard, { backgroundColor: theme.colors.surfaceMuted, borderRadius: cardRadius }]}><TextInput autoFocus value={rename} onChangeText={setRename} onSubmitEditing={() => { controller.renameNode(rename); setRenameOpen(false); }} style={[styles.renameInput, { color: theme.colors.textPrimary }]} /></View></AppBottomSheet></>; }

const styles = StyleSheet.create({ mapRoot: { flex: 1, minHeight: 420, overflow: 'hidden' }, canvasLayer: { position: 'absolute' }, invalidMap: { flex: 1, minHeight: 420, justifyContent: 'center' }, invalidState: { padding: 24, gap: 10, alignItems: 'center' }, nodeHit: { position: 'absolute' }, controls: { position: 'absolute', top: 12, right: 12, padding: 4, borderRadius: 12, borderWidth: StyleSheet.hairlineWidth }, controlRow: { flexDirection: 'row', alignItems: 'center', gap: 2 }, zoomRow: { marginTop: 3, paddingTop: 3, borderTopWidth: StyleSheet.hairlineWidth, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 2 }, controlButton: { width: 42, height: 40, borderRadius: 9, alignItems: 'center', justifyContent: 'center' }, zoomLabel: { minWidth: 42, textAlign: 'center' }, actionBar: { position: 'absolute', left: 10, right: 10, bottom: 28, minHeight: 48, borderRadius: 12, borderWidth: StyleSheet.hairlineWidth, paddingHorizontal: 5, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around' }, actionButton: { minWidth: 70, minHeight: 44, paddingHorizontal: 10, paddingVertical: 10, alignItems: 'center', justifyContent: 'center' }, outline: { paddingBottom: 120 }, outlineRow: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: 6 }, disclosure: { width: 16, color: '#6B7280' }, sheetCard: { overflow: 'hidden', paddingHorizontal: 16, paddingVertical: 6 }, actionRow: { minHeight: 56, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, emptySheetState: { paddingVertical: 18, textAlign: 'center' }, inputCard: { padding: 16 }, renameInput: { minHeight: 48, paddingHorizontal: 0, paddingVertical: 8, fontSize: 16, textAlignVertical: 'center' }, saveButton: { minHeight: 44, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 18, borderRadius: 12 } });
