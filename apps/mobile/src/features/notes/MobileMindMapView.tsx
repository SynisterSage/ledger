...
function NodeSheets({ controller, actionOpen, setActionOpen, renameOpen, setRenameOpen, moveOpen, setMoveOpen, rename, setRename }: { controller: Controller; actionOpen: boolean; setActionOpen: (value: boolean) => void; renameOpen: boolean; setRenameOpen: (value: boolean) => void; moveOpen: boolean; setMoveOpen: (value: boolean) => void; rename: string; setRename: (value: string) => void }) { const theme = useLedgerTheme(); const selected = controller.selected; return <><AppBottomSheet visible={actionOpen} onClose={() => setActionOpen(false)} title={<AppText variant="sectionTitle">{selected?.label ?? 'Map node'}</AppText>} snapPoints={['58%', '82%']} initialSnapPointIndex={0}><Pressable style={styles.actionRow} onPress={() => { setActionOpen(false); controller.addNode(false); }}><AppText variant="body">Add child</AppText></Pressable>{selected?.id !== controller.parsed.rootId ? <Pressable style={styles.actionRow} onPress={() => { setActionOpen(false); controller.addNode(true); }}><AppText variant="body">Add sibling</AppText></Pressable> : null}<Pressable style={styles.actionRow} onPress={() => { setRename(selected?.label ?? ''); setRenameOpen(true); setActionOpen(false); }}><AppText variant="body">Rename</AppText></Pressable><Pressable style={styles.actionRow} onPress={() => { controller.toggleComplete(); setActionOpen(false); }}><AppText variant="body">{selected?.completed ? 'Mark incomplete' : 'Complete'}</AppText></Pressable><Pressable style={styles.actionRow} onPress={() => { controller.toggleCollapse(); setActionOpen(false); }}><AppText variant="body">{selected?.collapsed ? 'Expand branch' : 'Collapse branch'}</AppText></Pressable>{selected?.id !== controller.parsed.rootId ? <Pressable style={styles.actionRow} onPress={() => { setActionOpen(false); setMoveOpen(true); }}><AppText variant="body">Move</AppText></Pressable> : null}{selected?.id !== controller.parsed.rootId ? <Pressable style={styles.actionRow} onPress={() => Alert.alert('Delete branch?', 'This removes the selected node and its descendants from the map.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Delete', style: 'destructive', onPress: controller.removeNode }])}><AppText variant="body" style={{ color: theme.colors.danger }}>Delete</AppText></Pressable> : null}</AppBottomSheet><AppBottomSheet visible={moveOpen} onClose={() => setMoveOpen(false)} title={<AppText variant="sectionTitle">Move node</AppText>} snapPoints={['58%', '82%']} initialSnapPointIndex={0}>{selected ? Object.values(controller.parsed.nodes).filter((node) => node.id !== selected.id && !controller.descendants(selected.id).has(node.id)).map((node) => <Pressable key={node.id} style={styles.actionRow} onPress={() => { controller.moveTo(node.id); setMoveOpen(false); }}><AppText variant="body">{node.label}</AppText></Pressable>) : null}</AppBottomSheet><AppBottomSheet visible={renameOpen} onClose={() => setRenameOpen(false)} title={<AppText variant="sectionTitle">Rename node</AppText>} snapPoints={['34%', '48%']} initialSnapPointIndex={0}><TextInput autoFocus value={rename} onChangeText={setRename} onSubmitEditing={() => { controller.renameNode(rename); setRenameOpen(false); }} style={[styles.renameInput, { color: theme.colors.textPrimary, backgroundColor: theme.colors.surfaceMuted, borderColor: theme.colors.borderSubtle }]} /><Pressable onPress={() => { controller.renameNode(rename); setRenameOpen(false); }} style={[styles.saveButton, { backgroundColor: theme.colors.accent }]}><AppText variant="button" style={{ color: theme.colors.onAccent }}>Save</AppText></Pressable></AppBottomSheet></>; }

const styles = StyleSheet.create({ mapRoot: { flex: 1, minHeight: 420, overflow: 'hidden' }, canvasLayer: { position: 'absolute' }, invalidMap: { flex: 1, minHeight: 420, justifyContent: 'center' }, invalidState: { padding: 24, gap: 10, alignItems: 'center' }, nodeHit: { position: 'absolute' }, controls: { position: 'absolute', top: 12, right: 12, padding: 4, borderRadius: 12, borderWidth: StyleSheet.hairlineWidth }, controlRow: { flexDirection: 'row', alignItems: 'center', gap: 2 }, zoomRow: { marginTop: 3, paddingTop: 3, borderTopWidth: StyleSheet.hairlineWidth, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 2 }, controlButton: { width: 42, height: 40, borderRadius: 9, alignItems: 'center', justifyContent: 'center' }, zoomLabel: { minWidth: 42, textAlign: 'center' }, actionBar: { position: 'absolute', left: 10, right: 10, bottom: 28, minHeight: 48, borderRadius: 12, borderWidth: StyleSheet.hairlineWidth, paddingHorizontal: 5, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around' }, actionButton: { minWidth: 70, minHeight: 44, paddingHorizontal: 10, paddingVertical: 10, alignItems: 'center', justifyContent: 'center' }, outline: { paddingBottom: 120 }, outlineRow: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: 6 }, disclosure: { width: 16, color: '#6B7280' }, actionRow: { minHeight: 50, justifyContent: 'center', borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#E5E7EB' }, renameInput: { minHeight: 48, borderWidth: StyleSheet.hairlineWidth, borderColor: '#E5E5E5', borderRadius: 16, paddingHorizontal: 16, paddingVertical: 0, fontSize: 16, textAlignVertical: 'center' }, saveButton: { marginTop: 14, alignItems: 'center', paddingVertical: 12, paddingHorizontal: 18, borderRadius: 10, backgroundColor: '#FF5F40' } });
