...
export const AgendaView = forwardRef<AgendaViewHandle, AgendaViewProps>(function AgendaView(
  {
    selectedDate,
    workspaceId,
    filters,
  ...
  const { items, isLoading, error, extendPast, extendFuture, retry } = useMobileAgendaItems(
    workspaceId,
    selectedDate,
    filters
  );
  const { entries } = useMemo(
    () => buildAgendaListEntries(items, selectedDate),
    [items, selectedDate]
  );
  const stickyHeaderIndices = useMemo(
    () => entries.flatMap((entry, index) => (entry.type === 'date_header' ? [index] : [])),
    [entries]
  );
    ...
    () =>
      new Map(
        entries.flatMap((entry, index) =>
          entry.type === 'date_header' ? [[entry.dateKey, index] as const] : []
        )
      ),
    [entries]
  );
  const getItemLayout = useCallback(
    (_: ArrayLike<AgendaListEntry> | null | undefined, index: number) => {
      const offset = entries
        .slice(0, index)
        .reduce(
          (total, entry) =>
            total +
            (entry.type === 'date_header'
              ? 38
              : entry.type === 'item'
              ? getAgendaRowHeight(entry.item)
              : 0),
          0
        );
  ...
  useEffect(() => {
    if (hasRestored || isLoading) return;
    const target = dateIndex.get(formatCalendarDateKey(new Date()));
    if (target === undefined) return;
    requestAnimationFrame(() => {
      listRef.current?.scrollToIndex({ index: target, animated: false, viewPosition: 0 });
      setHasRestored(true);
    });
  }, [dateIndex, hasRestored, isLoading]);
  ...
  const onViewableItemsChanged = useCallback(
    ({
      ...
      viewableItems: Array<{ item?: AgendaListEntry; isViewable?: boolean }>;
    }) => {
      // FlatList reports its provisional first rows before the initial
      // scroll-to-today completes. Do not let that transient row overwrite the
      // requested date and saved agenda anchor.
      if (!hasRestored) return;
      const dateHeader = viewableItems.find(
        (token) => token.isViewable !== false && token.item?.type === 'date_header'
      )?.item;
      if (!dateHeader || dateHeader.type !== 'date_header') return;
      const date = dateFromKey(dateHeader.dateKey);
      onChangeVisiblePeriod(date);
      onScrollStateChange?.({
        offset: scrollState?.offset ?? 0,
        anchorDateKey: dateHeader.dateKey,
      });
    },
    [hasRestored, onChangeVisiblePeriod, onScrollStateChange, scrollState?.offset]
  );

  const renderEntry = useCallback(
    ({ item }: { item: AgendaListEntry }) => {
        ...
        return (
          ...
          >
            <AppText
              variant="meta"
              style={{
                color: isToday(item.dateKey) ? theme.colors.accent : theme.colors.textPrimary,
                fontWeight: '600',
              }}
            >
              {dateLabel(item.dateKey)}
            </AppText>
          </View>
        );
      }
  ...
  return (
    <View style={styles.container}>
      {error ? (
        <View style={[styles.errorRow, { borderBottomColor: theme.colors.borderSubtle }]}>
