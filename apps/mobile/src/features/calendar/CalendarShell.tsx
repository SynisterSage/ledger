...
import { CalendarCreateSheet, CalendarSourcesSheet } from './CalendarSheets';
...
} from './useMobileCalendarState';
import {
  ContinuousMonthView,
  type ContinuousMonthViewHandle,
  type MonthScrollState,
} from './ContinuousMonthView';
...
function PlaceholderView({
  ...
  const labels: Record<MobileCalendarView, string> = {
    year: 'Year overview is ready for calendar content.',
    month: 'Month view is ready for calendar content.',
    agenda: 'Agenda view is ready for calendar content.',
    day: 'Day view is ready for calendar content.',
  };
  return (
    <View style={[styles.placeholder, { borderColor: theme.colors.borderSubtle }]}>
      <SymbolView
        name={{ ios: 'calendar', android: 'calendar_month', web: 'calendar_month' }}
        size={24}
        tintColor={theme.colors.textMuted}
      />
      <AppText variant="meta" style={styles.placeholderText}>
        {labels[view]}
      </AppText>
      <AppText variant="caption">
        {context.workspaceId === 'all' ? 'All workspaces' : 'Workspace calendar'}
      </AppText>
    </View>
  );
}

function AnimatedPeriodTitle({
  title,
  reduceMotion,
  style,
}: {
  ...
  return (
    <Animated.View style={[styles.periodTitleAnimation, { opacity, transform: [{ translateY }] }]}>
      <AppText variant="title" style={style}>
        {title}
      </AppText>
    </Animated.View>
  );
...
export function CalendarShell() {
  ...
  useEffect(() => {
    let active = true;
    const requestIdle =
      globalThis.requestIdleCallback ?? ((callback: () => void) => setTimeout(callback, 0));
    const cancelIdle =
  ...
  useEffect(() => {
    ...
    const isYearMonthTransition =
      (previousView === 'year' && calendar.view === 'month') ||
      (previousView === 'month' && calendar.view === 'year');
    ...
    Animated.parallel([
      Animated.timing(viewTransitionScale, {
        toValue: 1,
        duration: YEAR_VIEW_SCALE_DURATION,
        easing: Easing.out(Easing.cubic),
        useNativeDriver: true,
      }),
      Animated.timing(viewTransitionOpacity, {
        toValue: 1,
        duration: YEAR_VIEW_FADE_DURATION,
  ...
  const calendarViewMenuRef = useRef<CalendarViewMenuHandle>(null);
  ...
  const changeCalendarView = (nextView: MobileCalendarView) => {
    ...
    requestAnimationFrame(() => {
      if (nextView === 'agenda') {
        ...
        calendar.changeVisiblePeriod(today);
        setAgendaScrollStates((current) => ({
          ...current,
          [workspaceState.selectedWorkspaceId]: { offset: 0, anchorDateKey: todayKey },
        }));
        agendaViewRef.current?.scrollToDate(today);
      }
  ...
  const changeCalendarDisplay = (nextValue: MonthDisplayMode | 'list') => {
    if (nextValue === 'list') {
      const today = new Date();
      const todayKey = formatCalendarDateKey(today);
      calendar.selectDate(today);
      ...
      requestAnimationFrame(() => agendaViewRef.current?.scrollToDate(today));
      return;
    }

    const visibleMonth = calendar.visiblePeriod;
    calendar.setMonthDisplayMode(nextValue);
    calendar.setView('month');
    requestAnimationFrame(() => monthViewRef.current?.scrollToMonth(visibleMonth, false));
  };
  ...
  const handleCalendarItemAction = async (actionId: string, item: MobileCalendarItem) => {
    ...
    if (actionId === 'edit') {
      ...
      router.push({
        ...
        params: calendarEditorParams(item, workspaceState.selectedWorkspaceId),
      });
      return;
    }
    ...
    if (actionId === 'reschedule') {
      ...
      router.push({
        pathname: '/calendar/editor',
        params: { ...calendarEditorParams(item, workspaceState.selectedWorkspaceId), mode: 'edit' },
      });
      return;
    }
    if (actionId === 'meeting-note' && item.type === 'event') {
      try {
        await createMeetingNoteFromCalendar(item.workspaceId, {
          eventId: sourceId,
          provider: item.readOnly ? 'apple' : 'ledger',
          eventKey: sourceId,
          projectId: item.projectId,
        });
        setSelectedCalendarItem(null);
    ...
    if (actionId === 'snooze' && item.type === 'reminder') {
      await mobileRequest(`/api/reminders/${encodeURIComponent(sourceId)}/snooze`, {
        method: 'POST',
        headers: { 'x-workspace-id': item.workspaceId },
        body: JSON.stringify({ snooze_until: new Date(Date.now() + 60 * 60 * 1000).toISOString() }),
      });
      setSelectedCalendarItem(null);
      return;
    }
    if (actionId === 'delete') {
      const canReviewRelatedEvents =
        (item.type === 'event' || item.type === 'external_event') &&
        !item.readOnly &&
        (item.type === 'external_event' ||
          item.sourcePlatform === 'ics' ||
          Boolean(item.seriesId) ||
          Boolean(item.importSeriesKey));
        ...
        try {
          ...
          if (preview.matches.length > 0) {
            ...
            router.push({
              pathname: '/calendar/editor',
              params: {
                ...calendarEditorParams(item, workspaceState.selectedWorkspaceId),
                openDeleteMatches: '1',
              },
            });
            return;
      ...
      Alert.alert(`Delete ${item.type.replace('_', ' ')}?`, 'This cannot be undone.', [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            void (
              item.type === 'event' || item.type === 'external_event'
                ? deleteMobileEvent(item.workspaceId, sourceId)
                : item.type === 'reminder'
                ? deleteMobileReminder(item.workspaceId, sourceId)
                : deleteMobileTask(item.workspaceId, sourceId)
            ).then(() => {
              emitCalendarDataChanged(item.workspaceId);
              setSelectedCalendarItem(null);
              setCalendarItemActionMode(false);
            });
          },
    ...
    if (actionId === 'complete' && item.sourceId && item.type === 'reminder') {
      await mobileRequest(`/api/reminders/${encodeURIComponent(sourceId)}/complete`, {
        method: 'POST',
        headers: workspaceHeaders,
      });
    } else if (
      (actionId === 'complete' || actionId === 'focus') &&
      item.sourceId &&
      (item.type === 'task' || item.type === 'project_action')
    ) {
      await mobileRequest(`/api/tasks/${encodeURIComponent(sourceId)}`, {
        method: 'PATCH',
        headers: workspaceHeaders,
        body: JSON.stringify(
          actionId === 'focus'
            ? { show_in_today: true, is_today_focus: true }
            : { status: 'completed' }
        ),
      });
    } else if (actionId === 'complete' && item.type === 'event') {
      await updateMobileEvent(item.workspaceId, sourceId, {
        status: item.completed ? 'planned' : 'done',
      });
  ...
  return (
    ...
    >
      <View
        style={[
          styles.content,
          {
            ...
            paddingBottom: showLandscapeWeek
              ? 0
              : GLOBAL_TAB_BAR_HEIGHT + GLOBAL_TAB_BAR_GAP + CONTEXT_TOOLBAR_HEIGHT,
          },
        ]}
      >
        <View style={[styles.toolbar, showLandscapeWeek && styles.landscapeHidden]}>
          <Pressable
            ...
            accessibilityLabel={`Open year overview for ${calendar.visiblePeriod.getFullYear()}`}
            onPress={() =>
              calendar.view === 'month' ? calendar.setView('year') : calendar.setView('month')
            }
          ...
          >
            ...
            <AppText variant="bodyStrong">
              {calendar.view === 'year'
                ? formatPeriodTitle('year', calendar.visiblePeriod)
                : formatParentPeriod(calendar.view, calendar.visiblePeriod)}
            </AppText>
          </Pressable>
          <View style={styles.toolbarActions}>
            {calendar.view !== 'year' ? (
              <CalendarViewMenuController
                ref={calendarViewMenuRef}
                value={calendar.view === 'agenda' ? 'list' : calendar.monthDisplayMode}
                icon={{
                  ios:
                    calendar.view === 'agenda'
                      ? 'list.bullet'
                      : calendar.monthDisplayMode === 'compact'
                      ? 'rectangle.grid.2x2'
                      : calendar.monthDisplayMode === 'stacked'
                      ? 'rectangle.stack'
                      : 'calendar',
                  android:
                    calendar.view === 'agenda'
                ...
                onChange={changeCalendarDisplay}
              />
            ) : null}
            <Pressable
              accessibilityRole="button"
              ...
              onPress={() => {
                ...
                calendar.setCreationSheetOpen(true);
              }}
        ...
        <View style={[styles.periodHeader, showLandscapeWeek && styles.landscapeHidden]}>
          <AnimatedPeriodTitle
            ...
            style={calendar.view === 'year' ? styles.yearShellTitle : undefined}
          />
          <Pressable
            accessibilityRole="button"
          ...
          >
            <AppText variant="meta">
              {workspaceState.isLoading ? 'Loading workspace…' : workspaceLabel}
            </AppText>
            <SymbolView
              name={{
                ios: 'chevron.down',
        ...
        <Animated.View
          style={[
            styles.viewArea,
            { opacity: viewTransitionOpacity, transform: [{ scale: viewTransitionScale }] },
          ]}
        >
          {showLandscapeWeek ? (
            <LandscapeWeekView
              ...
              filters={calendar.filters}
              scrollOffset={
                dayScrollStates[workspaceState.selectedWorkspaceId]?.[
                  formatCalendarDateKey(calendar.selectedDate)
                ]
              }
              onScrollOffsetChange={(offset) => {
                setDayScrollStates((current) => ({
                  ...current,
                  [workspaceState.selectedWorkspaceId]: {
                    ...(current[workspaceState.selectedWorkspaceId] ?? {}),
                    [formatCalendarDateKey(calendar.selectedDate)]: offset,
                  },
                }));
              }}
              onSelectDate={(date) => {
              ...
              onLongPressItem={(item) => {
                setCalendarItemActionMode(true);
                setSelectedCalendarItem(item);
              }}
              onCreateAtTime={(date, minutes) => {
                calendar.selectDate(date);
                calendar.changeVisiblePeriod(date);
                setDayCreateTimeMinutes(minutes);
                calendar.setCreationSheetOpen(true);
              }}
              onBackToMonth={() => calendar.setView('month')}
              onChangeWeek={(amount) => {
                const next = new Date(calendar.selectedDate);
                next.setDate(next.getDate() + amount * 7);
                calendar.selectDate(next);
                calendar.changeVisiblePeriod(next);
              }}
              onOpenViewSheet={() => calendarViewMenuRef.current?.open()}
              onCreate={() => {
                setDayCreateTimeMinutes(null);
                calendar.setCreationSheetOpen(true);
              }}
            ...
            <View style={[styles.contentLoading, { backgroundColor: theme.colors.surfaceMuted }]} />
          ) : calendar.view === 'year' ? (
            <YearOverview
              ...
              onSelectMonth={(date) => {
                calendar.changeVisiblePeriod(date);
                calendar.setView('month');
                requestAnimationFrame(() => monthViewRef.current?.scrollToMonth(date));
              }}
              onSelectDate={(date) => {
                calendar.selectDate(date);
                calendar.changeVisiblePeriod(date);
              ...
              onVisibleYearChange={(year) =>
                calendar.changeVisiblePeriod(new Date(year, calendar.visiblePeriod.getMonth(), 1))
              }
            />
          ) : calendar.view === 'month' ? (
            <ContinuousMonthView
              ref={monthViewRef}
              mode={calendar.monthDisplayMode}
              selectedDate={calendar.selectedDate}
              visiblePeriod={calendar.visiblePeriod}
              workspaceId={workspaceState.selectedWorkspaceId}
              ...
              onSelectDate={(date) => {
                calendar.selectDate(date);
                calendar.changeVisiblePeriod(date);
                if (calendar.monthDisplayMode === 'details') changeCalendarView('day');
              }}
              onOpenDate={() => changeCalendarView('day')}
              onChangeVisiblePeriod={calendar.changeVisiblePeriod}
              onScrollStateChange={(state) => {
                setMonthScrollStates((current) => ({
                  ...current,
                  [workspaceState.selectedWorkspaceId]: state,
                }));
              }}
              onOpenItem={(item) => {
                setCalendarItemActionMode(false);
      ...
      {!isLandscape ? (
        <View
          style={[
            styles.contextToolbar,
            {
              bottom: insets.bottom + GLOBAL_TAB_BAR_HEIGHT + GLOBAL_TAB_BAR_GAP,
        ...
        >
          <Pressable
            ...
            onPress={handleToday}
            style={({ pressed }) => [styles.contextAction, { opacity: pressed ? 0.65 : 1 }]}
          >
            <AppText
              variant="meta"
              style={{ color: isTodayRelevant ? theme.colors.accent : theme.colors.textSecondary }}
            >
              Today
            </AppText>
          </Pressable>
          <Pressable
            accessibilityRole="button"
            accessibilityLabel="Choose visible calendars"
            onPress={() => calendar.setSourceSheetOpen(true)}
            style={({ pressed }) => [styles.contextAction, { opacity: pressed ? 0.65 : 1 }]}
          >
            <SymbolView
              name={{ ios: 'calendar', android: 'calendar_month', web: 'calendar_month' }}
          ...
          <Pressable
            accessibilityRole="button"
            accessibilityLabel="Open calendar inbox"
      ...
      <MonthCalendarItemSheet
        ...
        onAction={(actionId, item) => {
          void handleCalendarItemAction(actionId, item);
        }}
      />
      ...
      <CalendarCreateSheet
        ...
        initialStartAt={
          dayCreateTimeMinutes !== null
            ? (() => {
                const date = new Date(calendar.selectedDate);
                date.setHours(
                  Math.floor(dayCreateTimeMinutes / 60),
                  dayCreateTimeMinutes % 60,
                  0,
                  0
                );
        ...
        initialEndAt={
          dayCreateTimeMinutes !== null
            ? (() => {
                const date = new Date(calendar.selectedDate);
                date.setHours(
                  Math.floor(dayCreateTimeMinutes / 60) + 1,
                  dayCreateTimeMinutes % 60,
                  0,
                  0
                );
      ...
      <CalendarSourcesSheet
        visible={calendar.sourceSheetOpen}
        workspaceId={workspaceState.selectedWorkspaceId}
        workspaceLabel={workspaceLabel}
        filters={calendar.filters}
        onChangeFilters={calendar.setFilters}
        onResetFilters={calendar.resetFilters}
        onOpenWorkspacePicker={() => {
          calendar.setSourceSheetOpen(false);
        ...
        onClose={() => calendar.setSourceSheetOpen(false)}
      />
...
const styles = StyleSheet.create({
  ...
  contextToolbar: {
    position: 'absolute',
    left: CALENDAR_PAGE_PADDING,
    right: CALENDAR_PAGE_PADDING,
    height: CONTEXT_TOOLBAR_HEIGHT,
