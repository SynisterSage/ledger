...
export function ProjectFilterSheet({ visible, filters, showOwnership = true, activeCount, onChange, onReset, onClose }: ProjectFilterSheetProps) {
  ...
  return <AppBottomSheet visible={visible} onClose={onClose} snapPoints={['72%', '88%']} initialSnapPointIndex={0} title={<View style={styles.title}><AppText variant="sectionTitle">Filter projects</AppText>{activeCount ? <AppText variant="caption">{activeCount} active</AppText> : null}</View>} headerAccessory={<Pressable accessibilityRole="button" accessibilityLabel="Done" onPress={onClose} hitSlop={8}><SymbolView name={{ ios: 'checkmark', android: 'check', web: 'check' }} size={22} tintColor={theme.colors.accent} /></Pressable>} contentStyle={{ gap: 18 }}>
    {activeCount ? <Pressable accessibilityRole="button" onPress={onReset} hitSlop={8}><AppText variant="caption" style={{ color: theme.colors.accent }}>Reset filters</AppText></Pressable> : null}
