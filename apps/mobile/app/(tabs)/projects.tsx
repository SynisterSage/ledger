...
export default function ProjectsScreen() {
  ...
  const handleProjectLongPress = (project: MobileProjectsProject) => Alert.alert(project.name, undefined, [
    { text: 'Open', onPress: () => router.push(`/project/${project.id}`) },
    ...(permissions.canAddAction ? [{ text: 'Add action', onPress: () => router.push({ pathname: '/capture/project-action', params: { projectId: project.id, returnTo: `/project/${project.id}` } }) }] : []),
    ...(permissions.canAddMilestone ? [{ text: 'Add milestone', onPress: () => router.push({ pathname: '/project/milestone-new', params: { projectId: project.id } }) }] : []),
    ...(permissions.canAddNote ? [{ text: 'Add note', onPress: () => router.push({ pathname: '/capture/note', params: { projectId: project.id } }) }] : []),
  ...
  return <Screen contentStyle={{ paddingTop: 0 }}>
    <View style={styles.fill}>
      ...
      <NewProjectSheet visible={newProjectOpen} onClose={() => setNewProjectOpen(false)} onCreated={(projectId) => { if (projectId) router.push(`/project/${projectId}`); else void load(true); }} />
      ...
      <Animated.ScrollView style={styles.fill} contentContainerStyle={{ paddingTop: TODAY_HEADER_SCROLL_SPACE, paddingBottom: theme.spacing['3xl'] + 132 }} refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={refresh} tintColor={theme.colors.accent} />} onScroll={Animated.event([{ nativeEvent: { contentOffset: { y: scrollY } } }], { useNativeDriver: true })} scrollEventThrottle={16} showsVerticalScrollIndicator={false}>
        <View style={styles.content}>
          ...
          <View style={styles.toolbar}><View style={[styles.switcher, { borderColor: theme.colors.borderSubtle }]}>{(['projects', 'milestones'] as const).map((mode) => <Pressable key={mode} onPress={() => setViewMode(mode)} style={[styles.switcherItem, viewMode === mode && { backgroundColor: theme.colors.surfaceMuted }]}><AppText variant="caption" style={{ color: viewMode === mode ? theme.colors.textPrimary : theme.colors.textMuted, fontWeight: '600' }}>{mode === 'projects' ? 'Projects' : 'Milestones'}</AppText></Pressable>)}</View>{permissions.canCreate ? <Pressable accessibilityRole="button" accessibilityLabel="Create project" onPress={() => setNewProjectOpen(true)}><AppText variant="caption" style={{ color: theme.colors.accent }}>+ New</AppText></Pressable> : null}</View>
...
function ProjectSections({ sections, hasFilters, onClear, collapsed, setCollapsed, onProjectPress, onProjectLongPress, canCreate, onCreate }: { sections: Record<ProjectSection, MobileProjectsProject[]>; hasFilters: boolean; onClear: () => void; collapsed: Record<ProjectSection, boolean>; setCollapsed: (value: Record<ProjectSection, boolean>) => void; onProjectPress: (id: string) => void; onProjectLongPress?: (project: MobileProjectsProject) => void; canCreate: boolean; onCreate: () => void }) {
  ...
  if (!visible.length) return hasFilters ? <View style={styles.emptyFilter}><AppText variant="meta">No projects match these filters</AppText><Pressable onPress={onClear}><AppText variant="caption" style={{ color: theme.colors.accent }}>Clear filters</AppText></Pressable></View> : <View style={styles.emptyFilter}><AppText variant="meta">No projects yet</AppText><AppText variant="caption">Create a project to start organizing your work.</AppText>{canCreate ? <Pressable onPress={onCreate}><AppText variant="caption" style={{ color: theme.colors.accent }}>Create a project</AppText></Pressable> : null}</View>;
...
function MilestoneList({ groups, hasAnyMilestones, completedCollapsed, onToggleCompleted, hasFilters, onClear, onProjectPress, onLongPress }: { groups: MobileProjectDateGroup[]; hasAnyMilestones: boolean; completedCollapsed: boolean; onToggleCompleted: () => void; hasFilters: boolean; onClear: () => void; onProjectPress: (item: MobileProjectDate) => void; onLongPress: (item: MobileProjectDate) => void }) {
  const theme = useLedgerTheme();
  if (!groups.length) return hasFilters ? <View style={styles.emptyFilter}><AppText variant="meta">No milestones match these filters</AppText><Pressable onPress={onClear}><AppText variant="caption" style={{ color: theme.colors.accent }}>Clear filters</AppText></Pressable></View> : <View style={styles.emptyFilter}><AppText variant="meta">No upcoming milestones</AppText>{!hasAnyMilestones ? <Pressable onPress={() => Alert.alert('Add milestone', 'Milestone creation is coming soon.')}><AppText variant="caption" style={{ color: theme.colors.accent }}>Add milestone</AppText></Pressable> : null}</View>;
...
function ProjectListSkeleton() { const theme = useLedgerTheme(); return <View style={styles.skeleton}><View style={styles.skeletonSummary}><Skeleton width={170} height={16} radius={5} /><Skeleton width={100} height={16} radius={5} /></View><Skeleton width={170} height={30} radius={8} /><Skeleton width="100%" height={14} radius={4} />{[1, 2, 3].map((item) => <Skeleton key={item} width="100%" height={58} radius={5} />)}<AppText variant="meta" style={{ color: theme.colors.textMuted }}>Loading projects…</AppText></View>; }
const styles = StyleSheet.create({ fill: { flex: 1 }, content: { paddingHorizontal: 2, gap: 20 }, summary: { flexDirection: 'row', alignItems: 'center', minHeight: 34 }, summaryItem: { flexDirection: 'row', alignItems: 'baseline', gap: 5, paddingHorizontal: 10 }, toolbar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, switcher: { alignSelf: 'flex-start', flexDirection: 'row', borderWidth: StyleSheet.hairlineWidth, borderRadius: 8, padding: 2 }, switcherItem: { paddingVertical: 7, paddingHorizontal: 15, borderRadius: 6 }, filters: { gap: 18, paddingVertical: 1 }, filter: { paddingVertical: 5 }, filterSummary: { flexDirection: 'row', alignItems: 'center', gap: 10 }, filterSummaryText: { flex: 1 }, sections: { gap: 26 }, section: { gap: 10 }, sectionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, milestoneRow: { minHeight: 58, flexDirection: 'row', alignItems: 'stretch', gap: 10, paddingVertical: 4 }, milestoneDateColumn: { width: 62, justifyContent: 'flex-start', paddingTop: 3 }, timelineColumn: { width: 12, alignItems: 'center', position: 'relative' }, timelineMarker: { width: 7, height: 7, marginTop: 5, borderRadius: 1, transform: [{ rotate: '45deg' }], zIndex: 1 }, timelineLine: { position: 'absolute', top: 13, bottom: -5, width: StyleSheet.hairlineWidth }, milestoneCopy: { minWidth: 0, flex: 1, gap: 3 }, milestoneTitleRow: { minWidth: 0, flexDirection: 'row', alignItems: 'center', gap: 7 }, emptyFilter: { alignItems: 'flex-start', gap: 7, paddingVertical: 24 }, state: { paddingVertical: 24 }, errorState: { gap: 8, paddingVertical: 24 }, refreshError: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, skeleton: { gap: 12, paddingVertical: 8 }, skeletonSummary: { flexDirection: 'row', justifyContent: 'space-between' }, });
