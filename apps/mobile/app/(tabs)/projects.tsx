...
const projectsCacheKey = (workspaceId: string) => `mobile:projects:${workspaceId}`;
...
export default function ProjectsScreen() {
  ...
  const load = useCallback(async (silent = false, force = false) => {
    ...
    const requestedWorkspaceId = workspaceState.selectedWorkspaceId;
    if (!silent) { setIsLoading(true); setError(null); }
    try {
      setRefreshError(null);
      const response = await getMobileResource(
        projectsCacheKey(requestedWorkspaceId),
        () => getMobileProjects(requestedWorkspaceId, true),
        { force },
      );
      if (loadToken !== loadTokenRef.current || requestedWorkspaceId !== workspaceState.selectedWorkspaceId) return;
      ...
      if (!silent) setError(message); else setRefreshError(message);
    } finally {
      if (loadToken === loadTokenRef.current && !silent) setIsLoading(false);
    }
  }, [workspaceState.selectedWorkspaceId]);

  useEffect(() => { void bootstrapWorkspaceState(); }, []);
  useEffect(() => {
    const cached = readMobileResource<Awaited<ReturnType<typeof getMobileProjects>>>(projectsCacheKey(workspaceState.selectedWorkspaceId));
    if (!cached) return;
  ...
  }, [load, resetScrollState, workspaceState.selectedWorkspaceId]));
  const refresh = useCallback(async () => { setIsRefreshing(true); try { await load(true, true); } finally { setIsRefreshing(false); } }, [load]);
...
function ProjectSections({ sections, hasFilters, onClear, collapsed, setCollapsed, onProjectPress, onProjectLongPress, canCreate, onCreate }: { sections: Record<ProjectSection, MobileProjectsProject[]>; hasFilters: boolean; onClear: () => void; collapsed: Record<ProjectSection, boolean>; setCollapsed: (value: Record<ProjectSection, boolean>) => void; onProjectPress: (id: string) => void; onProjectLongPress?: (project: MobileProjectsProject) => void; canCreate: boolean; onCreate: () => void }) {
  ...
  if (!visible.length) {
    if (hasFilters) {
      return <EmptyState iconName={{ ios: 'line.3.horizontal.decrease.circle', android: 'filter_alt', web: 'filter_alt' }} title="No projects match these filters" description="Clear the filters to see the full project list." kind="no-results" density="compact" style={styles.emptyState} primaryAction={{ label: 'Clear filters', variant: 'link', onPress: onClear }} />;
    }
    return <EmptyState iconName={{ ios: 'folder', android: 'folder_open', web: 'folder_open' }} title="A place for the work ahead" description="Create a project to gather the next actions, notes, and milestones that move something forward." kind={canCreate ? 'first-use' : 'permission'} density="compact" style={styles.emptyState} primaryAction={canCreate ? { label: 'Create project', onPress: onCreate } : undefined} />;
  }
...
function MilestoneList({ groups, hasAnyMilestones, canCreate, onCreate, completedCollapsed, onToggleCompleted, hasFilters, onClear, onProjectPress, onLongPress }: { groups: MobileProjectDateGroup[]; hasAnyMilestones: boolean; canCreate: boolean; onCreate: () => void; completedCollapsed: boolean; onToggleCompleted: () => void; hasFilters: boolean; onClear: () => void; onProjectPress: (item: MobileProjectDate) => void; onLongPress: (item: MobileProjectDate) => void }) {
  ...
  if (!groups.length) {
    if (hasFilters) {
      return <EmptyState iconName={{ ios: 'line.3.horizontal.decrease.circle', android: 'filter_alt', web: 'filter_alt' }} title="No milestones match these filters" description="Clear the filters to see the full milestone list." kind="no-results" density="compact" style={styles.emptyState} primaryAction={{ label: 'Clear filters', variant: 'link', onPress: onClear }} />;
    }
    return <EmptyState iconName={{ ios: 'calendar', android: 'event', web: 'event' }} title="Keep important dates visible" description={hasAnyMilestones ? 'There are no upcoming milestones right now.' : 'Add milestones to keep project deadlines and meaningful checkpoints in view.'} kind={!hasAnyMilestones && canCreate ? 'first-use' : 'informational'} density="compact" style={styles.emptyState} primaryAction={!hasAnyMilestones && canCreate ? { label: 'Create milestone', onPress: onCreate } : undefined} />;
  }
