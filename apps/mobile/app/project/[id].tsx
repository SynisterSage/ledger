...
import { hasIntegrationProviderIcon, IntegrationProviderIcon } from '@/features/projects/IntegrationProviderIcon';
...
export default function MobileProjectScreen() {
  ...
  return <Screen contentStyle={{ paddingTop: theme.spacing.lg }} {...projectFadeProps}>
    ...
    <ScrollView ref={scrollRef} refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={refresh} tintColor={theme.colors.accent} />} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <View onLayout={(event) => setTasksY(event.nativeEvent.layout.y)}><Section title="Project plan" action={permissions.canAddAction || permissions.canAddMilestone ? '+ Add' : undefined} onAction={permissions.canAddAction || permissions.canAddMilestone ? openActions : undefined}>{detail.sectionErrors.tasks ? <SectionFailure message={detail.sectionErrors.tasks} onRetry={() => void load(true)} /> : planItems.length ? <View style={[styles.planCard, { backgroundColor: theme.colors.surfaceMuted, borderRadius: theme.radius.window }]}>{planItems.map((planItem, index) => <ProjectPlanCard key={`${planItem.kind}:${planItem.item.id}`} planItem={planItem} projectColor={project.color || theme.colors.accent} canComplete={planItem.kind === 'action' ? permissions.canAddAction : permissions.canAddMilestone} isLast={index === planItems.length - 1} onComplete={() => planItem.kind === 'action' ? completeTask(planItem.item) : completeMilestone(planItem.item.id)} onPress={() => { if (planItem.kind === 'action') { Alert.alert(planItem.item.title, planItem.item.due_date ? `Due ${dateLabel(planItem.item.due_date)}` : 'No due date'); return; } Alert.alert(planItem.item.title, [planItem.date ? `Due ${planItem.date}` : 'No date', 'Milestone'].join(' · '), planItem.completed || !permissions.canAddMilestone ? [{ text: 'Done' }] : [{ text: 'Complete', onPress: () => completeMilestone(planItem.item.id) }, { text: 'Cancel', style: 'cancel' }]); }} />)}</View> : <ProjectEmptyState iconName={{ ios: 'checklist', android: 'checklist', web: 'checklist' }} title="Nothing planned yet" description={permissions.canAddAction || permissions.canAddMilestone ? 'Add an action or milestone to give this project a clear next step.' : 'There are no actions or milestones in this project.'} kind={permissions.canAddAction || permissions.canAddMilestone ? 'first-use' : 'permission'} actionLabel={permissions.canAddAction || permissions.canAddMilestone ? 'Add to plan' : undefined} onAction={permissions.canAddAction || permissions.canAddMilestone ? openActions : undefined} />}</Section></View>
...
function ProjectPlanCard({ planItem, projectColor, canComplete, isLast, onComplete, onPress }: { planItem: ProjectPlanItem; projectColor: string; canComplete: boolean; isLast: boolean; onComplete: () => void; onPress: () => void }) {
  const theme = useLedgerTheme();
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={`${title}, ${metadata}`} accessibilityHint="Opens item details" onPress={onPress} onLongPress={canComplete && !planItem.completed ? () => Alert.alert(title, undefined, [{ text: 'Complete', onPress: onComplete }, { text: 'Cancel', style: 'cancel' }]) : undefined} style={({ pressed }) => [styles.planRow, isLast ? styles.planRowLast : null, { opacity: pressed ? 0.68 : planItem.completed ? 0.58 : 1 }]}>
    ...
    {planItem.completed ? <SymbolView name={{ ios: 'checkmark.circle.fill', android: 'check_circle', web: 'check_circle' }} size={16} tintColor={theme.colors.success} /> : null}
  </Pressable>;
}
