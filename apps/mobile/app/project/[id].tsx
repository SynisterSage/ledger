...
import { deleteMobileProject, updateMobileProject, updateMobileProjectMilestone } from '@/api/projects';
...
export default function MobileProjectScreen() {
  ...
  const [actionsY, setActionsY] = useState<number | null>(null);
  ...
  const nextActions = openTasks.slice(0, 5);
  const completedTasks = detail?.tasks.filter(isCompleteTask).length ?? 0;
  ...
  return <Screen contentStyle={{ paddingTop: theme.spacing.lg }}>
    ...
    <ScrollView ref={scrollRef} refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={refresh} tintColor={theme.colors.accent} />} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {project.attention ? <Pressable onPress={() => actionsY !== null && scrollRef.current?.scrollTo({ y: Math.max(0, actionsY - 16), animated: true })} style={[styles.attention, { borderLeftColor: project.attention.severity === 'critical' ? theme.colors.danger : theme.colors.warning }]}><SymbolView name={{ ios: 'exclamationmark', android: 'priority_high', web: 'priority_high' }} size={15} tintColor={project.attention.severity === 'critical' ? theme.colors.danger : theme.colors.warning} /><AppText variant="meta" style={{ color: project.attention.severity === 'critical' ? theme.colors.danger : theme.colors.warning, flex: 1 }}>{project.attention.label}</AppText></Pressable> : null}

      <View onLayout={(event) => setActionsY(event.nativeEvent.layout.y)}><SectionHeader title="Next actions" action={permissions.canAddAction ? '+ Add' : undefined} onAction={permissions.canAddAction ? () => router.push({ pathname: '/capture/project-action', params: { projectId: project.id, workspaceId: project.workspace_id, returnTo: `/project/${project.id}` } }) : undefined} />
        {detail.sectionErrors.tasks ? <SectionFailure message={detail.sectionErrors.tasks} onRetry={() => void load(true)} /> : nextActions.length ? nextActions.map((task) => <TodayItemRow key={task.id} type="project_action" title={task.title} metadata={[task.due_date ? `Due ${dateLabel(task.due_date)}` : 'No due date']} status={isOverdueTask(task) ? 'overdue' : 'active'} onPress={() => Alert.alert(task.title, task.due_date ? `Due ${dateLabel(task.due_date)}` : 'No due date')} onComplete={permissions.canAddAction ? () => completeTask(task) : undefined} onLongPress={permissions.canAddAction ? () => Alert.alert(task.title, undefined, [{ text: 'Complete', onPress: () => completeTask(task) }, { text: 'Cancel', style: 'cancel' }]) : undefined} />) : <EmptyInline title="No next action" detail="Add one to keep this project moving" />}
        {openTasks.length > 5 ? <InlineAction label="View all tasks" onPress={() => Alert.alert('Tasks', 'The full project task view is coming soon.')} /> : null}
      </View>

      {detail.tasks.length || detail.sectionErrors.tasks ? <Section title="Tasks" action={detail.sectionErrors.tasks ? undefined : `${openTasks.length} open`}>{detail.sectionErrors.tasks ? <SectionFailure message={detail.sectionErrors.tasks} onRetry={() => void load(true)} /> : <><AppText variant="caption">To do and in progress work · {completedTasks} completed</AppText><InlineAction label="View all tasks" onPress={() => Alert.alert('Tasks', 'The full project task view is coming soon.')} /></>}</Section> : null}
      {detail.milestones.length ? <Section title="Milestones" action={permissions.canAddMilestone ? "+ Add" : undefined} onAction={permissions.canAddMilestone ? () => router.push({ pathname: '/project/milestone-new', params: { projectId: project.id, workspaceId: project.workspace_id } }) : undefined}><View style={styles.simpleRows}>{milestones.map((milestone) => <Pressable key={milestone.id} onPress={() => router.push(`/project/${project.id}?milestoneId=${milestone.id}`)} onLongPress={() => permissions.canAddMilestone ? Alert.alert(milestone.title, `${dateLabel(milestone.milestone_date)} · Milestone`, [{ text: 'Mark complete', onPress: () => completeMilestone(milestone.id) }, { text: 'Cancel', style: 'cancel' }]) : undefined} style={styles.simpleRow}><View style={[styles.smallDot, { backgroundColor: milestone.milestone_date && milestone.milestone_date < new Date().toISOString().slice(0, 10) ? theme.colors.warning : project.color || theme.colors.accent }]} /><View style={styles.rowCopy}><AppText variant="body" numberOfLines={1}>{milestone.title}</AppText><AppText variant="caption">{dateLabel(milestone.milestone_date)} · Milestone</AppText></View></Pressable>)}</View>{detail.milestones.filter((item) => !item.completed).length > 4 ? <InlineAction label="View all milestones" onPress={() => router.push(`/project/${project.id}?milestones=all`)} /> : null}</Section> : null}
      {detail.notes.length || detail.sectionErrors.notes ? <Section title="Notes" action={detail.sectionErrors.notes || !permissions.canAddNote ? undefined : "+ New"} onAction={permissions.canAddNote ? () => router.push({ pathname: '/capture/note', params: { projectId: project.id, workspaceId: project.workspace_id } }) : undefined}>{detail.sectionErrors.notes ? <SectionFailure message={detail.sectionErrors.notes} onRetry={() => void load(true)} /> : <View style={styles.simpleRows}>{detail.notes.slice(0, 4).map((note) => <Pressable key={note.id} onPress={() => router.push('/capture/note')} style={styles.simpleRow}><SymbolView name={{ ios: 'note.text', android: 'note', web: 'note' }} size={16} tintColor={theme.colors.textMuted} /><View style={styles.rowCopy}><AppText variant="body" numberOfLines={1}>{note.title}</AppText><AppText variant="caption">Updated {dateLabel(note.updated_at) ?? 'recently'}</AppText></View></Pressable>)}</View>}</Section> : null}
      ...
      {detail.activity.length ? <Section title="Activity"><View style={styles.simpleRows}>{detail.activity.slice(0, 5).map((item) => <View key={item.id} style={styles.activityRow}><AppText variant="caption" style={styles.rowCopy}>{item.title}</AppText><AppText variant="caption">{dateLabel(item.timestamp)}</AppText></View>)}</View></Section> : null}
    </ScrollView>
  </Screen>;
}
