...
export const routeAskLedgerMessage = (
  ...
  const workspaceResourceStateQuestion = workspaceResourceWords.test(normalized)
    && (
      ...
      || (/\bprojects?\b/i.test(normalized) && workspaceCreativeProjectSignals.test(normalized))
      || (workspaceContentReferenceSignals.test(normalized) && explicitExistingResourceSignals.test(normalized))
    )
    && (
      ...
      || workspaceContentReferenceSignals.test(normalized)
