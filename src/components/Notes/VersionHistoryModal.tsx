      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200 bg-white px-5 py-4">
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-gray-500">Version history</p>
            <p className="mt-1 truncate text-sm font-semibold text-gray-900">{noteTitle || 'Untitled note'}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          {isLoading ? (
            ...
            <div className="space-y-2">
              {versions.map((version, index) => {
                ...
                return (
                  <div key={version.id} className="rounded-xl border border-gray-200 bg-white px-3 py-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs text-gray-500">
                          {formatVersionStamp(version.created_at)} · {resolveActorName(version.versioned_by)} · {version.reason || 'update'}
                        </p>
                        <p className="mt-1 truncate text-xs text-gray-600">{preview || 'No content preview'}</p>
                      </div>
                      <button
                        type="button"
