...
const ConnectedProviderIcon = ({ provider, size = 14 }: { provider?: string | null; size?: number }) => {
  if (String(provider ?? '').toLowerCase().includes('google')) return <GoogleDriveIcon size={size} />;
  return normalizeIntegrationProvider(provider) ? <IntegrationProviderMark provider={provider} size={size} className="object-contain" /> : <Link2 size={size} />;
};
