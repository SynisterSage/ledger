...
export function GoogleDriveRuleBuilderModal({ sources, initialRule, onClose, onSave, busy }: { sources: GoogleDriveRuleSource[]; initialRule?: GoogleDriveRule | null; onClose: () => void; onSave: (sourceId: string, payload: Record<string, unknown>, ruleId?: string) => Promise<void>; busy: boolean }) {
  ...
  return <ModalOverlay isOpen onClose={onClose} backdropBorderRadius="inherit" disablePortal manageWindowChrome={false} classNameContent="overflow-hidden" classNameContainer="w-full max-w-2xl overflow-hidden rounded-[var(--ledger-surface-radius)] border border-[var(--ledger-border-subtle)] bg-[var(--ledger-surface-card)] shadow-[var(--ledger-shadow)]">
