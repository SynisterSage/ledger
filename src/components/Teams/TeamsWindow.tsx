...
export const TeamsWindow = () => {
  ...
  const [inviteAddTeamIdDraft, setInviteAddTeamIdDraft] = useState<string>('none');
  ...
  const handleCreateTeam = (event: FormEvent) => {
    ...
    const creatorName = user?.email?.split('@')[0] || user?.id || 'You';
    const creator: TeamMember = {
      id: user?.id || 'current-user',
      name: creatorName,
      email: user?.email ?? null,
      initials: getInitials(creatorName, user?.email ?? null),
      role: 'lead',
    };
  ...
  const handleInvite = (event: FormEvent) => {
    ...
    const targetTeamId = inviteTeamId ?? (inviteAddTeamIdDraft === 'none' ? null : inviteAddTeamIdDraft);
    if (targetTeamId) {
      ...
      setTeams((current) =>
        current.map((team) =>
          team.id === targetTeamId ? { ...team, members: [...team.members, pendingMember] } : team
        )
      );
    }
    ...
    setInviteAddTeamIdDraft('none');
    setInviteTeamId(null);
    setIsInviteOpen(false);
  };

  const addMemberToTeam = (member: TeamMember) => {
    if (!addMemberTeamId) return;
    setTeams((current) =>
      current.map((team) =>
        team.id === addMemberTeamId
          ? {
              ...team,
              members: team.members.some((item) => item.id === member.id)
                ? team.members
                : [...team.members, member],
            }
          : team
  ...
  const deleteTeam = (teamId: string) => {
    ...
    setSelectedTeamId((current) => (current === teamId ? teams.find((item) => item.id !== teamId)?.id ?? null : current));
    setOpenedTeamId((current) => (current === teamId ? null : current));
    setContextMenu(null);
  };

  const openInviteForTeam = (teamId: string | null) => {
    setInviteTeamId(teamId);
    setInviteAddTeamIdDraft(teamId ?? 'none');
    setIsInviteOpen(true);
  };

  const assignedRows = openedTeam
    ? [
        {
          icon: Circle,
          title: 'Review catalog PDF',
          meta: `Task · ${openedTeam.activeProjects[0] ?? workspaceName} · Due Jul 3`,
        },
        {
          icon: Diamond,
          title: 'Finish Meta Steps Exhibition',
          meta: `Milestone · Jun 30 · ${openedTeam.activeProjects[0] ?? workspaceName}`,
        },
        {
          icon: BriefcaseBusiness,
          title: openedTeam.activeProjects[0] ?? 'Assign first project',
          meta: openedTeam.activeProjects.length > 0 ? 'Project · 95% · Due Oct 23' : 'Project · No team work yet',
        },
      ]
    : [];
                    ...
                    <div>
                      ...
                      {openedTeam.assignedCount > 0 ? (
                        assignedRows.map((row) => (
                          ...
                          >
                            <row.icon size={14} className="text-[var(--ledger-text-muted)]" />
                            <div className="min-w-0">
                              <p className={teamsTheme.title}>{row.title}</p>
                              <p className={teamsTheme.meta}>{row.meta}</p>
                            </div>
                          </div>
                        ))
                      ) : (
                        <div className="px-4 py-10">
                          ...
                          <button type="button" className={`mt-4 ${teamsTheme.action}`}>
                            Assign work
                          </button>
                        </div>
                      )}
      ...
      >
        <form onSubmit={handleInvite} className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--ledger-text-muted)]">
              {inviteTeam
                ? `They will join ${workspaceName} and be added to this team.`
          ...
          {!inviteTeam ? (
            <label className="block space-y-1.5">
              <span className={teamsTheme.label}>Add to team</span>
              <select
                value={inviteAddTeamIdDraft}
                onChange={(event) => setInviteAddTeamIdDraft(event.target.value)}
                className={teamsTheme.modalInput}
              >
