...
export default function SlackWindow({ routeWorkspaceId = null }: SlackWindowProps) {
  ...
  useEffect(() => {
    const handleMarkAllRead = () => { void markAllActivityRead(); };
    window.addEventListener('slack:mark-all-read', handleMarkAllRead);
    return () => window.removeEventListener('slack:mark-all-read', handleMarkAllRead);
  });
  ...
  const openIntake = (id?: string | null) => {
    if (!id) {
      if (window.desktopWindow) void window.desktopWindow.openModule('inbox');
      return;
    }
  ...
  const markAllActivityRead = async () => {
    if (!workspaceId || activityBusy || !activities.some((activity) => !activity.is_read)) return;
    setActivityBusy('read-all');
    try {
      await api.markAllSlackActivityRead(workspaceId, { date: activityDate, filter: activityFilter === 'unread' ? 'all' : activityFilter });
      setActivities((current) => current.map((activity) => ({ ...activity, is_read: true })));
    } catch {
      setActivityError('Ledger could not mark Slack activity as read.');
    } finally {
...
function SlackActivitySection({ date, onDateChange, activities, filter, onFilterChange, isLoading, error, onRetry, onOpenSlack, onRead, onSendToIntake, onLinkContext, onOpenIntake, busy, identityConnected, needsReauthorization, onConnect, onReauthorize }: { date: string; onDateChange: (date: string) => void; recap: SlackRecap | null; activities: SlackActivity[]; filter: string; onFilterChange: (filter: string) => void; isLoading: boolean; error: string | null; onRetry: () => void; onOpenSlack: (url?: string | null) => void; onRead: (activity: SlackActivity) => void; onSendToIntake: (activity: SlackActivity) => void; onLinkContext: (activity: SlackActivity) => void; onOpenIntake: (id?: string | null) => void; busy: string | null; identityConnected: boolean; needsReauthorization: boolean; onConnect: () => void; onReauthorize: () => void }) {
  ...
  return <section className="space-y-3 pt-3"><div className="flex justify-end"><button type="button" onClick={() => window.dispatchEvent(new CustomEvent('slack:mark-all-read'))} disabled={Boolean(busy) || !activities.some((activity) => !activity.is_read)} className="rounded-md px-2 py-1 text-[11px] font-medium text-[var(--ledger-text-muted)] hover:bg-[var(--ledger-surface-muted)] hover:text-[var(--ledger-text-primary)] disabled:opacity-40">Mark all read</button></div>
...
function SlackContextRail({ status, identity, identityLoading, identityError, workspaceName, onManage, onOpenSlack, onOpenIntake, onConnect, onDisconnect, identityBusy, canConnect, recap }: { status: SlackStatus | null; identity: SlackIdentity | null; identityLoading: boolean; identityError: string | null; workspaceName?: string; onManage: () => void; onOpenSlack: (url?: string | null) => void; onOpenIntake: () => void; onConnect: () => void; onDisconnect: () => void; identityBusy: 'connect' | 'disconnect' | null; canConnect: boolean; recap: SlackRecap | null }) {
  ...
  return <aside className="flex min-h-0 flex-col overflow-hidden border-l border-[var(--ledger-border-subtle)] bg-[var(--ledger-surface-card)] shadow-none"><div className="sticky top-0 z-10 flex h-12 shrink-0 items-center justify-between border-b border-[var(--ledger-border-subtle)] bg-[var(--ledger-surface-card)] px-4"><span className="text-sm font-medium text-[var(--ledger-text-primary)]">Slack context</span><span className="text-xs text-[var(--ledger-text-muted)]">{status?.team_name || workspaceName || 'Connected'}</span></div><div className="min-h-0 flex-1 overflow-y-auto px-4 py-4"><div className="space-y-5"><section><p className="text-[11px] font-medium uppercase tracking-wide text-[var(--ledger-text-muted)]">Today</p>{hasActivity ? <div className="mt-2 space-y-1.5 text-xs">{metrics?.new_messages ? <div className="flex justify-between"><span className="text-[var(--ledger-text-muted)]">New messages</span><span className="font-medium">{metrics.new_messages}</span></div> : null}{metrics?.mentions || metrics?.replies ? <div className="flex justify-between"><span className="text-[var(--ledger-text-muted)]">Mentions and replies</span><span className="font-medium">{(metrics.mentions ?? 0) + (metrics.replies ?? 0)}</span></div> : null}{metrics?.active_threads ? <div className="flex justify-between"><span className="text-[var(--ledger-text-muted)]">Active threads</span><span className="font-medium">{metrics.active_threads}</span></div> : null}</div> : <p className="mt-1.5 text-sm text-[var(--ledger-text-muted)]">Nothing new today</p>}</section><section className="border-t border-[var(--ledger-border-subtle)] pt-4"><p className="text-[11px] font-medium uppercase tracking-wide text-[var(--ledger-text-muted)]">Your Slack identity</p>{identityLoading ? <div className="mt-2 h-10 animate-pulse rounded-lg bg-[var(--ledger-surface-muted)]" /> : isConnected ? <div className="mt-2 flex items-center gap-2.5 rounded-lg border border-[var(--ledger-border-subtle)] px-2.5 py-2"><UserAvatar user={{ displayName, email: identity?.slack_email ?? null, avatarUrl: identity?.slack_avatar_url ?? null, avatarUpdatedAt: null }} size="sm" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-[var(--ledger-text-primary)]">{displayName}</p><p className="text-[11px] text-emerald-700">Connected</p></div><button type="button" onClick={onDisconnect} disabled={Boolean(identityBusy)} className="rounded-md px-2 py-1 text-[11px] text-[var(--ledger-text-muted)] hover:bg-[var(--ledger-surface-muted)]">{identityBusy === 'disconnect' ? 'Disconnecting…' : 'Disconnect'}</button></div> : <div className="mt-2 flex items-center justify-between rounded-lg border border-[var(--ledger-border-subtle)] px-2.5 py-2"><span className="text-sm text-[var(--ledger-text-muted)]">Not connected</span>{canConnect ? <button type="button" onClick={onConnect} disabled={Boolean(identityBusy)} className="rounded-md px-2 py-1 text-[11px] font-medium text-[var(--ledger-text-secondary)] hover:bg-[var(--ledger-surface-muted)]">Connect</button> : null}</div>}{identityError ? <p className="mt-2 text-xs text-red-700">{identityError}</p> : null}</section><section className="border-t border-[var(--ledger-border-subtle)] pt-4"><p className="text-[11px] font-medium uppercase tracking-wide text-[var(--ledger-text-muted)]">Actions</p><div className="mt-2 space-y-1"><button type="button" onClick={onManage} className="flex w-full items-center gap-2 rounded-md px-1 py-1.5 text-left text-xs text-[var(--ledger-text-secondary)] hover:bg-[var(--ledger-surface-muted)]"><Settings2 size={13} /> Manage integration</button><button type="button" onClick={() => onOpenSlack()} className="flex w-full items-center gap-2 rounded-md px-1 py-1.5 text-left text-xs text-[var(--ledger-text-secondary)] hover:bg-[var(--ledger-surface-muted)]"><ExternalLink size={13} /> Open Slack</button></div></section><section className="border-t border-[var(--ledger-border-subtle)] pt-4"><p className="text-[11px] font-medium uppercase tracking-wide text-[var(--ledger-text-muted)]">Capture destination</p><button type="button" onClick={onOpenIntake} className="mt-1 text-sm font-medium text-[var(--ledger-text-secondary)] hover:text-[var(--ledger-text-primary)] hover:underline">{workspaceName || 'This workspace'} Intake</button></section></div></div></aside>;
}

export function LegacySlackContextRail({ status, identity, identityLoading, identityError, workspaceName, onManage, onOpenSlack, onOpenIntake, onConnect, onDisconnect, identityBusy, canConnect, recap }: { status: SlackStatus | null; identity: SlackIdentity | null; identityError: string | null; workspaceName?: string; onManage: () => void; onOpenSlack: (url?: string | null) => void; onOpenIntake: () => void; onConnect: () => void; onDisconnect: () => void; identityBusy: 'connect' | 'disconnect' | null; canConnect: boolean; recap: SlackRecap | null; identityLoading: boolean }) {
  const metrics = recap?.metrics ?? { new_messages: 0, mentions: 0, replies: 0, active_threads: 0, sent_to_intake: 0, linked_contexts: 0 };
  const hasActivity = Boolean(metrics?.new_messages || metrics?.mentions || metrics?.replies || metrics?.active_threads);
  const isConnected = identity?.status === 'connected';
  const displayName = identity?.slack_display_name || identity?.slack_real_name || 'Slack member';
  return <aside className="border-t border-[var(--ledger-border-subtle)] px-4 py-4 md:border-l md:border-t-0"><div className="text-xs"><div className="flex items-start gap-2.5 border-b border-[var(--ledger-border-subtle)] pb-4"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-[#E01E5A]/10"><IntegrationProviderMark provider="slack" size={16} /></span><div className="min-w-0"><p className="text-sm font-medium text-[var(--ledger-text-primary)]">Slack</p><p className="mt-0.5 truncate text-xs text-[var(--ledger-text-muted)]">{status?.team_name || 'Slack workspace'} · Connected</p></div></div><div className="pt-5"><p className="text-xs font-medium text-[var(--ledger-text-secondary)]">Today</p>{hasActivity ? <div className="mt-2 space-y-1.5">{metrics?.new_messages ? <div className="flex items-center justify-between gap-3"><span className="text-[var(--ledger-text-muted)]">New messages</span><span className="font-medium text-[var(--ledger-text-primary)]">{metrics.new_messages}</span></div> : null}{metrics?.mentions || metrics?.replies ? <p className="text-[var(--ledger-text-muted)]"><span className="font-medium text-[var(--ledger-text-primary)]">{metrics.mentions ?? 0} mentions</span> · <span className="font-medium text-[var(--ledger-text-primary)]">{metrics.replies ?? 0} replies</span></p> : null}</div> : <p className="mt-1.5 text-xs text-[var(--ledger-text-muted)]">Nothing new today</p>}</div><div className="pt-5"><p className="text-xs font-medium text-[var(--ledger-text-secondary)]">Your Slack identity</p>{identityLoading ? <div className="mt-2 h-9 animate-pulse rounded-lg bg-[var(--ledger-surface-muted)]" /> : isConnected ? <div className="mt-2 flex items-center gap-2.5 rounded-lg border border-[var(--ledger-border-subtle)] px-2.5 py-2"><UserAvatar user={{ displayName, email: identity?.slack_email ?? null, avatarUrl: identity?.slack_avatar_url ?? null, avatarUpdatedAt: null }} size="sm" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-[var(--ledger-text-primary)]">{displayName}</p><p className="text-[11px] text-emerald-700">Connected</p></div><button type="button" onClick={onDisconnect} disabled={Boolean(identityBusy)} className="rounded-md px-2 py-1 text-[11px] text-[var(--ledger-text-muted)] hover:bg-[var(--ledger-surface-muted)] hover:text-[var(--ledger-text-primary)]">{identityBusy === 'disconnect' ? 'Disconnecting…' : 'Disconnect'}</button></div> : <div className="mt-2 flex items-center justify-between rounded-lg border border-[var(--ledger-border-subtle)] px-2.5 py-2"><span className="text-sm text-[var(--ledger-text-muted)]">Not connected</span>{canConnect ? <button type="button" onClick={onConnect} disabled={Boolean(identityBusy)} className="rounded-md px-2 py-1 text-[11px] font-medium text-[var(--ledger-text-secondary)] hover:bg-[var(--ledger-surface-muted)]">Connect</button> : null}</div>}{identityError ? <p className="mt-2 text-red-700">{identityError}</p> : null}</div><div className="mt-5 space-y-2"><button type="button" onClick={onManage} className="flex w-full items-center gap-2 rounded-md px-1 py-1 text-left text-[var(--ledger-text-secondary)] hover:bg-[var(--ledger-surface-muted)] hover:text-[var(--ledger-text-primary)]"><Settings2 size={13} /> Manage integration</button><button type="button" onClick={() => onOpenSlack()} className="flex w-full items-center gap-2 rounded-md px-1 py-1 text-left text-[var(--ledger-text-secondary)] hover:bg-[var(--ledger-surface-muted)] hover:text-[var(--ledger-text-primary)]"><ExternalLink size={13} /> Open Slack</button></div><div className="mt-6 border-t border-[var(--ledger-border-subtle)] pt-3"><p className="text-[11px] text-[var(--ledger-text-muted)]">Sending captures to</p><button type="button" onClick={onOpenIntake} className="mt-0.5 text-xs font-medium text-[var(--ledger-text-secondary)] hover:text-[var(--ledger-text-primary)] hover:underline">{workspaceName || 'this workspace'} Intake</button></div></div></aside>;
