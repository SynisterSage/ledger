  ...
  const openGoogleDrivePicker = async () => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => {
        const docsView = new googleWindow.google.picker.DocsView(googleWindow.google.picker.ViewId.DOCS).setMode(googleWindow.google.picker.DocsViewMode.LIST);
        const pickerBuilder = new googleWindow.google.picker.PickerBuilder().addView(docsView).enableFeature(googleWindow.google.picker.Feature.MULTISELECT_ENABLED).setOAuthToken(tokenResult.access_token).setOrigin(window.location.origin);
