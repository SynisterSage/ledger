...
const resourceLabel = (reference: Reference) => { const value = String(reference.metadata?.pageName ?? '').trim(); return value; };
const referenceTitle = (reference: Reference, fallbackTitle?: string | null) => String(reference.metadata?.nodeName ?? reference.metadata?.fileName ?? fallbackTitle ?? 'Figma design');
const referenceFileName = (reference: Reference, fallbackFileName?: string | null) => String(reference.metadata?.fileName ?? fallbackFileName ?? '').trim();

export function LinkedDesignsSection({ target, canEdit = true, canInsert = false, onInsert, fallbackNodeName, fallbackFileName }: { target: LinkedDesignTarget; canEdit?: boolean; canInsert?: boolean; onInsert?: (reference: { id: string; url: string }) => void; fallbackNodeName?: string | null; fallbackFileName?: string | null }) {
  const api = useApi();
    ...
    try {
      ...
      const hydrated = await Promise.all(rows.map(async (link) => {
        ...
        if (!preview?.url && reference?.provider === 'figma') {
          ...
          } catch { /* Consent or connection may still be required. */ }
        }
  ...
  return <section className="space-y-2 border-t border-[color:var(--ledger-border-subtle)] pt-4" aria-labelledby={`linked-designs-${target.targetId}`}>
    ...
    {loading ? <div className="flex items-center gap-2 py-3 text-xs text-[var(--ledger-text-muted)]"><Loader2 size={13} className="animate-spin" />Loading linked designs…</div> : rows.length === 0 ? <div className="rounded-lg border border-dashed border-[color:var(--ledger-border-subtle)] px-3 py-3 text-xs text-[var(--ledger-text-muted)]">Keep related Figma files and frames connected to this work.{canEdit && <button type="button" className="ml-1 font-medium text-[var(--ledger-accent)]" onClick={() => setDialogOpen(true)}>Link design</button>}</div> : <div className="space-y-1">{rows.map(({ link, reference }) => { const preview = previews[link.external_reference_id]; const refUrl = reference?.normalized_url || reference?.external_url || ''; const title = referenceTitle(reference!, fallbackNodeName); const fileName = referenceFileName(reference!, fallbackFileName); const context = [resourceLabel(reference!), fileName, preview?.capturedAt ? `Preview captured ${formatDate(preview.capturedAt)}` : ''].filter(Boolean).join(' · '); return <div key={link.id} className="relative flex items-center gap-2 rounded-lg border border-[color:var(--ledger-border-subtle)] px-2.5 py-2"><div className="flex h-9 w-12 shrink-0 items-center justify-center overflow-hidden rounded-md bg-[var(--ledger-surface-muted)]">{preview?.url ? <img src={preview.url} alt={title} className="h-full w-full object-cover" /> : <FileImage size={15} className="text-[var(--ledger-text-muted)]" />}</div><button type="button" className="min-w-0 flex-1 text-left" onClick={() => refUrl && openExternal(refUrl)}><p className="truncate text-xs font-medium text-[var(--ledger-text-primary)]">{title}</p><p className="truncate text-[11px] text-[var(--ledger-text-muted)]">Figma{context ? ` · ${context}` : ''}</p></button>{canInsert && onInsert && <button type="button" title="Insert into description" aria-label="Insert into description" onClick={() => onInsert({ id: reference!.id, url: refUrl })} className="rounded-md p-1 text-[var(--ledger-text-muted)] hover:bg-[var(--ledger-surface-hover)]"><Check size={13} /></button>}<button type="button" aria-label="Design actions" onPointerDown={(event) => event.stopPropagation()} onClick={() => setMenuLink(menuLink?.id === link.id ? null : link)} className="rounded-md p-1 text-[var(--ledger-text-muted)] hover:bg-[var(--ledger-surface-hover)]"><MoreHorizontal size={14} /></button>{busyId === link.id && <Loader2 size={13} className="animate-spin text-[var(--ledger-accent)]" />}<div className="sr-only">{link.sources?.join(', ')}</div>{menuLink?.id === link.id && <div className="absolute right-2 top-10 z-20 w-44 rounded-lg border border-[color:var(--ledger-border-subtle)] bg-[var(--ledger-surface-card)] p-1 shadow-[var(--ledger-shadow)]" onPointerDown={(event) => event.stopPropagation()}><button type="button" className="block w-full rounded-md px-2 py-1.5 text-left text-xs hover:bg-[var(--ledger-surface-hover)]" onClick={() => { openExternal(refUrl); setMenuLink(null); }}>Open in Figma</button><button type="button" disabled={!canEdit} className="block w-full rounded-md px-2 py-1.5 text-left text-xs hover:bg-[var(--ledger-surface-hover)] disabled:opacity-50" onClick={() => { void refresh(link); setMenuLink(null); }}>Refresh preview</button><button type="button" className="block w-full rounded-md px-2 py-1.5 text-left text-xs hover:bg-[var(--ledger-surface-hover)]" onClick={() => { void navigator.clipboard.writeText(refUrl); setMenuLink(null); }}>Copy Figma link</button>{canInsert && onInsert && <button type="button" className="block w-full rounded-md px-2 py-1.5 text-left text-xs hover:bg-[var(--ledger-surface-hover)]" onClick={() => { onInsert({ id: reference!.id, url: refUrl }); setMenuLink(null); }}>Insert into description</button>}<button type="button" className="block w-full rounded-md px-2 py-1.5 text-left text-xs hover:bg-[var(--ledger-surface-hover)]" onClick={() => { void showLocations(reference!.id); setMenuLink(null); }}>View linked locations</button>{canEdit && <button type="button" className="block w-full rounded-md px-2 py-1.5 text-left text-xs text-[var(--ledger-danger)] hover:bg-[color:rgba(217,45,32,0.08)]" onClick={() => { void unlink(link); setMenuLink(null); }}>Unlink from this item</button>}</div>}</div>; })}</div>}
