...
export const ExternalEmbedRenderer = ({ nodeKey, externalReferenceId, externalUrl }: { nodeKey: NodeKey; externalReferenceId: string; externalUrl: string }) => {
  ...
  const refresh = async () => {
    ...
    setRefreshing(true);
    try {
      if (reference?.provider === 'github' || reference?.provider === 'google_drive') { await api.resolveExternalReference(externalReferenceId); setAccessStatus('accessible'); toast.show(`${reference.provider === 'google_drive' ? 'Google Drive file' : 'GitHub reference'} refreshed`, { variant: 'success' }); return; }
    ...
    } catch { toast.show(reference?.provider === 'github' ? 'GitHub could not be reached.' : 'Ledger couldn’t refresh this Figma preview.', { variant: 'error' }); }
  ...
  const fileName = String(metadata.name ?? metadata.fileName ?? (googleDrive ? 'Google Drive file' : 'Figma design'));
  ...
  const statusMessage = accessStatus === 'connection_required' ? `${googleDrive ? 'Connect Google Drive to view file details.' : `${github ? 'GitHub' : 'Figma'} is not connected for this workspace.`}` : accessStatus === 'repository_not_approved' ? 'Repository access changed.' : accessStatus === 'revoked' || accessStatus === 'expired' ? `${googleDrive ? 'Google Drive connection expired.' : `${github ? 'GitHub' : 'Figma'} access needs to be refreshed.`}` : accessStatus === 'inaccessible' ? `This ${googleDrive ? 'Google Drive file' : github ? 'GitHub item' : 'Figma design'} is not accessible.` : accessStatus === 'not_found' ? `This ${googleDrive ? 'file' : github ? 'GitHub item' : 'Figma design'} is no longer available.` : accessStatus === 'error' ? `Ledger couldn’t load this ${googleDrive ? 'Google Drive file' : github ? 'GitHub reference' : 'Figma preview'}.` : null;
