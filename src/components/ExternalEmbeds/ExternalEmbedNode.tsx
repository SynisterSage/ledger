...
export const ExternalEmbedRenderer = ({ nodeKey, externalReferenceId, externalUrl }: { nodeKey: NodeKey; externalReferenceId: string; externalUrl: string }) => {
  ...
  const refresh = async () => {
    ...
    setRefreshing(true);
    try {
      if (reference?.provider === 'github') { await api.resolveExternalReference(externalReferenceId); setAccessStatus('accessible'); toast.show('GitHub reference refreshed', { variant: 'success' }); return; }
