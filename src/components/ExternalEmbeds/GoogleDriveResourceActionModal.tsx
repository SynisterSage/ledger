...
export function GoogleDriveResourceActionModal({ action, title, currentParentId, onClose, onSubmit }: { action: GoogleDriveAction; title: string; currentParentId?: string; onClose: () => void; onSubmit: (payload: { name?: string; destination_folder_id?: string; expected_current_parent_id?: string }) => Promise<void> }) {
  ...
  return <ModalOverlay isOpen onClose={onClose} backdropBorderRadius="inherit" disablePortal manageWindowChrome={false} classNameContainer="w-full max-w-md overflow-hidden rounded-[var(--ledger-surface-radius)] border border-[color:var(--ledger-border-subtle)] bg-[var(--ledger-surface-card)] p-5 shadow-[var(--ledger-shadow)]"><h2 className="text-base font-semibold text-[var(--ledger-text-primary)]">{heading}</h2><p className="mt-1 text-xs text-[var(--ledger-text-muted)]">{title}</p>
