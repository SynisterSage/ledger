...
export const decodeAskLedgerContext = (value: string | null | undefined): AskLedgerInitialContext | null => {
  ...
  try {
    ...
    return {
      ...
      title: parsed.title,
      ...(parsed.contextType === 'project' ? { contextType: 'project' as const } : parsed.contextType === 'meeting' ? { contextType: 'meeting' as const } : parsed.contextType === 'notes_home' ? { contextType: 'notes_home' as const } : {}),
      ...
      ...(parsed.origin === 'projects' ? { origin: 'projects' as const } : parsed.origin === 'notes_home' ? { origin: 'notes_home' as const } : {}),
