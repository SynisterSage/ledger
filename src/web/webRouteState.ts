...
export const parseWebLocation = (location: Pick<Location, 'pathname' | 'search'>, historyState?: unknown): ParsedWebLocation => {
  ...
  switch (remainder) {
    ...
    default: {
      ...
      const settings = remainder.match(/^settings\/(?:(workspace)\/)?(workspace|members|teams|security-audit|data-privacy|calendar|notifications|sidebar|local-ai|meeting-notes|integrations|developer-api)(?:\/(google-drive|github|slack|figma))?$/);
      if (settings) {
        const section = settings[3] ?? settings[2];
        return { kind: 'route', route: { kind: 'workspace', workspaceId, page: 'settings', scope: 'workspace', section: section === 'meeting-notes' ? 'meeting-notes' : section as 'workspace' | 'members' | 'teams' | 'security-audit' | 'data-privacy' | 'calendar' | 'notifications' | 'sidebar' | 'local-ai' | 'integrations' | 'developer-api' | 'google-drive' | 'github' | 'slack' | 'figma' } };
      }
      return { kind: 'unknown' };
    }
  }
