...
export const useApi = () => {
  ...
  return useMemo(
    () => ({
      ...
      connectGoogleDriveFolderToProject: (projectId: string, folderId: string, resourceKey?: string | null) => request(`/api/projects/${encodeURIComponent(projectId)}/connected-sources/google-drive`, { method: 'POST', body: JSON.stringify({ folder_id: folderId, ...(resourceKey ? { resource_key: resourceKey } : {}) }) }),
