-- Migration: 140_fix_atomic_calendar_event_bulk_delete_id_ambiguity
-- Description: Qualify event columns in the bulk-delete function because its
-- return column is also named id.
...
BEGIN
  IF COALESCE(array_length(p_event_ids, 1), 0) = 0
     OR (
       SELECT COUNT(*)
       FROM public.events
       WHERE public.events.workspace_id = p_workspace_id
         AND public.events.id = ANY(p_event_ids)
