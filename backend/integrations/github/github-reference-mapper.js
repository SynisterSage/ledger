...
export const mapGithubPullRequestContext = ({ reviews = [], checkRuns = [], statuses = [], requestedReviewers = [] }) => {
  ...
  const failingRuns = runs.filter((run) => ['failure', 'failed', 'error', 'action_required', 'startup_failure'].includes(String(run.conclusion ?? '').toLowerCase())).length;
