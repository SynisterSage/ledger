...
export const registerTemplateRoutes = (app, supabase, { authMiddleware, rateLimit, resolveWorkspaceIdForRequest, normalizeNullableText, htmlToPlainText, normalizeNoteHtml, plainTextToParagraphHtml }) => {
  ...
  app.post('/api/notes/from-template/:templateId', authMiddleware, rateLimit('write'), async (req, res) => {
    try {
      const workspaceId = await resolveWorkspaceIdForRequest(req)
  ...
  app.post('/api/templates/from-note/:noteId', authMiddleware, rateLimit('write'), async (req, res) => {
    try {
      const workspaceId = await resolveWorkspaceIdForRequest(req)
  ...
  app.patch('/api/templates/:id/set-default', authMiddleware, rateLimit('write'), async (req, res) => {
    try {
      const workspaceId = await resolveWorkspaceIdForRequest(req)
      const templateId = String(req.params.id)
