...
Phase 3 remains read-only. Do not request Contents, Actions, Administration, Workflows, organization permissions, or any write permission. Do not subscribe to push, commit, review-comment, or other unrelated activity.
...
Apply migrations `095_github_app_phase1.sql`, `096_github_external_references_phase2.sql`, `098_note_preview_and_metadata_queries.sql`, and `099_github_phase3_live_awareness.sql` before testing. Phase 3 acknowledges and deduplicates webhook deliveries through the existing generic `integration_webhook_events` table and only updates GitHub objects already linked to Ledger work.
